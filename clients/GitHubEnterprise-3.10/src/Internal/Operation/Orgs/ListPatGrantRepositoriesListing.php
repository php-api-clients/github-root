...
final class ListPatGrantRepositoriesListing
{
    ...
    public function __construct(private readonly SchemaValidator $responseSchemaValidator, private readonly Internal\Hydrator\Operation\Orgs\Org\PersonalAccessTokens\PatId\Repositories $hydrator, string $org, int $patId, int $perPage = 30, int $page = 1)
    {
        $this->org     = $org;
        $this->patId   = $patId;
    ...
    {
        return new Request('GET', str_replace(['{org}', '{pat_id}', '{per_page}', '{page}'], [$this->org, $this->patId, $this->perPage, $this->page], '/orgs/{org}/personal-access-tokens/{pat_id}/repositories' . '?per_page={per_page}&page={page}'));
    }
