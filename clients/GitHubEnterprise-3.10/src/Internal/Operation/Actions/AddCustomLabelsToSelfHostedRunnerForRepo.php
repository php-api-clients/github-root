...
final class AddCustomLabelsToSelfHostedRunnerForRepo
{
    ...
    public function __construct(private readonly SchemaValidator $requestSchemaValidator, private readonly SchemaValidator $responseSchemaValidator, private readonly Internal\Hydrator\Operation\Repos\Owner\Repo\Actions\Runners\RunnerId\Labels $hydrator, string $owner, string $repo, int $runnerId)
    {
        $this->owner    = $owner;
        $this->repo     = $repo;
    ...
    public function createRequest(array $data): RequestInterface
    {
        ...
        return new Request('POST', str_replace(['{owner}', '{repo}', '{runner_id}'], [$this->owner, $this->repo, $this->runnerId], '/repos/{owner}/{repo}/actions/runners/{runner_id}/labels'), ['Content-Type' => 'application/json'], json_encode($data));
    }
