...
final class Four
{
    ...
    /** @return |iterable<Schema\HookDeliveryItem>|Schema\ApplicationGrant|array{code:int}|iterable<Schema\AuditLogEvent>|iterable<Schema\GistComment>|iterable<Schema\GistCommit>|iterable<Schema\GistSimple>|Schema\GitignoreTemplate|iterable<Schema\GhesGetMaintenance>|iterable<Schema\GhesVersion>|Schema\Thread|iterable<Schema\Event>|iterable<Schema\OrgHook>|iterable<Schema\Issue>|iterable<Schema\SimpleUser>|iterable<Schema\Migration>|iterable<Schema\Package>|iterable<Schema\OrganizationProgrammaticAccessGrantRequest>|iterable<Schema\OrganizationProgrammaticAccessGrant>|iterable<Schema\OrgPreReceiveHook>|iterable<Schema\Project>|iterable<Schema\MinimalRepository>|iterable<Schema\RepositoryFineGrainedPermission>|iterable<Schema\TeamSimple>|iterable<Schema\Team>|Schema\ProjectColumn|iterable<Schema\ProjectColumn>|Schema\ScimEnterpriseGroupList|Schema\ScimEnterpriseUserList|Schema\ConfigurationStatus|Schema\MaintenanceStatus|Schema\EnterpriseSettings|iterable<Schema\TeamDiscussion>|iterable<Schema\TeamProject>|Schema\GpgKey|Schema\Key|iterable<Schema\OrgMembership>|Schema\SshSigningKey|iterable<Schema\BaseGist>|iterable<Schema\GpgKey>|iterable<Schema\KeySimple>|iterable<Schema\OrganizationSimple>|iterable<Schema\SocialAccount>|iterable<Schema\SshSigningKey>|Schema\StarredRepository|Schema\Repository */
                ...
                if ($pathChunks[2] === 'settings') {
                    ...
                    if ($pathChunks[3] === 'all') {
                        ...
                        if ($call === 'GET /enterprise/stats/security-products') {
                            return $this->routers->router🔀Get🔀EnterpriseAdmin()->getSecurityProducts($params);
                        }
                ...
                if ($pathChunks[2] === 'v1') {
                    if ($pathChunks[3] === 'maintenance') {
                        if ($call === 'GET /manage/v1/maintenance') {
                            return $this->routers->router🔀Get🔀EnterpriseAdmin()->getManageMaintenance($params);
                        }
                    } elseif ($pathChunks[3] === 'version') {
                        if ($call === 'GET /manage/v1/version') {
                            return $this->routers->router🔀Get🔀EnterpriseAdmin()->getVersion($params);
                        }
                    }
                }
                ...
                if ($pathChunks[2] === '{org}') {
                    if ($pathChunks[3] === 'announcement') {
                        ...
                        if ($call === 'GET /orgs/{org}/custom-repository-roles') {
                            return $this->routers->router🔀Get🔀Orgs()->listCustomRepoRoles($params);
                        }
                        ...
                        if ($call === 'GET /orgs/{org}/packages') {
                            return $this->routers->router🔀Get🔀Packages()->listPackagesForOrganization($params);
                        }
                    } elseif ($pathChunks[3] === 'personal-access-token-requests') {
                        if ($call === 'GET /orgs/{org}/personal-access-token-requests') {
                            return $this->routers->router🔀Get🔀Orgs()->listPatGrantRequests($params);
                        }
                    } elseif ($pathChunks[3] === 'personal-access-tokens') {
                        if ($call === 'GET /orgs/{org}/personal-access-tokens') {
                            return $this->routers->router🔀Get🔀Orgs()->listPatGrants($params);
                        }
                        ...
                        if ($call === 'GET /orgs/{org}/repository-fine-grained-permissions') {
                            return $this->routers->router🔀Get🔀Orgs()->listRepoFineGrainedPermissions($params);
                        }
                ...
                if ($pathChunks[2] === 'docker') {
                    if ($pathChunks[3] === 'conflicts') {
                        if ($call === 'GET /user/docker/conflicts') {
                            return $this->routers->router🔀Get🔀Packages()->listDockerMigrationConflictingPackagesForAuthenticatedUser($params);
                        }
                    }
                ...
                if ($pathChunks[2] === '{username}') {
                    if ($pathChunks[3] === 'events') {
                        ...
                        if ($call === 'GET /users/{username}/packages') {
                            return $this->routers->router🔀Get🔀Packages()->listPackagesForUser($params);
                        }
                    } elseif ($pathChunks[3] === 'projects') {
                        ...
                        if ($call === 'GET /users/{username}/social_accounts') {
                            return $this->routers->router🔀Get🔀Users()->listSocialAccountsForUser($params);
                        }
