...
final class Four
{
    ...
    /** @return |Schema\GistComment|array{code:int}|Schema\BaseGist|iterable<Schema\GhesSetMaintenanceResponse>|Schema\ProjectColumn|Schema\GroupResponse|Schema\UserResponse|Schema\MaintenanceStatus */
                ...
                if ($pathChunks[2] === 'v1') {
                    if ($pathChunks[3] === 'maintenance') {
                        if ($call === 'POST /manage/v1/maintenance') {
                            return $this->routers->router🔀Post🔀EnterpriseAdmin()->setManageMaintenance($params);
                        }
                    }
                }
                ...
                if ($pathChunks[2] === '{org}') {
                    if ($pathChunks[3] === 'custom-repository-roles') {
                        if ($call === 'POST /orgs/{org}/custom-repository-roles') {
                            return $this->routers->router🔀Post🔀Orgs()->createCustomRepoRole($params);
                        }
