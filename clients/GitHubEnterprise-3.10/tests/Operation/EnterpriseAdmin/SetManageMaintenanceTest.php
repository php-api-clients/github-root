...
final class SetManageMaintenanceTest extends AsyncTestCase
{
    ...
    public function call_httpCode_400_requestContentType_application_json_empty(): void
    {
        ...
        $browser->request('POST', '/manage/v1/maintenance', Argument::type('array'), Schema\GhesSetMaintenanceRequest::SCHEMA_EXAMPLE_DATA)->willReturn(resolve($response))->shouldBeCalled();
    ...
    public function operations_httpCode_400_requestContentType_application_json_empty(): void
    {
        ...
        $browser->request('POST', '/manage/v1/maintenance', Argument::type('array'), Schema\GhesSetMaintenanceRequest::SCHEMA_EXAMPLE_DATA)->willReturn(resolve($response))->shouldBeCalled();
        $client = new Client($auth->reveal(), $browser->reveal());
        $result = await($client->operations()->enterpriseAdmin()->setManageMaintenance(json_decode(Schema\GhesSetMaintenanceRequest::SCHEMA_EXAMPLE_DATA, true)));
