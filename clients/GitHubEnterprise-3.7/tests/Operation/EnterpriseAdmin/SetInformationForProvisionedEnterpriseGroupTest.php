...
use ApiClients\Client\GitHubEnterprise\Operation\EnterpriseAdmin\SetInformationForProvisionedEnterpriseGroup;
...
final class SetInformationForProvisionedEnterpriseGroupTest extends AsyncTestCase
{
    ...
    {
        $response = new Response(200, ['Content-Type' => 'application/scim+json'], Schema\ScimEnterpriseGroupResponse::SCHEMA_EXAMPLE_DATA);
        $auth     = $this->prophesize(AuthenticationInterface::class);
        $auth->authHeader(Argument::any())->willReturn('Bearer beer')->shouldBeCalled();
        $browser = $this->prophesize(Browser::class);
        $browser->withBase(Argument::any())->willReturn($browser->reveal());
        $browser->withFollowRedirects(Argument::any())->willReturn($browser->reveal());
        $browser->request('PUT', '/scim/v2/Groups/generated_null', Argument::type('array'), Schema\Group::SCHEMA_EXAMPLE_DATA)->willReturn(resolve($response))->shouldBeCalled();
        $client = new Client($auth->reveal(), $browser->reveal());
        $client->call(SetInformationForProvisionedEnterpriseGroup::OPERATION_MATCH, (static function (array $data): array {
