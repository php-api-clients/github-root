...
namespace ApiClients\Client\GitHubEnterprise\Internal\Hydrator\Operation\Repos\Owner\Repo\Pages;
...
class Deployment implements ObjectMapper
{
    ...
    private function hydrateApiClients⚡️Client⚡️GitHubEnterprise⚡️Schema⚡️PageDeployment(array $payload): PageDeployment
    {
        $properties    = [];
        $missingFields = [];
        try {
        ...
        if (count($missingFields) > 0) {
            throw UnableToHydrateObject::dueToMissingFields(PageDeployment::class, $missingFields, stack: $this->hydrationStack);
        }

        try {
            return new PageDeployment(...$properties);
        } catch (Throwable $exception) {
            throw UnableToHydrateObject::dueToError('ApiClients\Client\GitHubEnterprise\Schema\PageDeployment', $exception, stack: $this->hydrationStack);
        }
    }
