...
final class SetCustomOidcSubClaimForRepo
{
    ...
    public function __construct(private readonly SchemaValidator $requestSchemaValidator, private readonly SchemaValidator $responseSchemaValidator, private readonly Internal\Hydrator\Operation\Repos\Owner\Repo\Actions\Oidc\Customization\Sub $hydrator, string $owner, string $repo)
    {
        $this->owner = $owner;
        $this->repo  = $repo;
    }
    ...
    {
        $this->requestSchemaValidator->validate($data, Reader::readFromJson(Schema\Actions\SetCustomOidcSubClaimForRepo\Request\ApplicationJson::SCHEMA_JSON, \cebe\openapi\spec\Schema::class));

        return new Request('PUT', str_replace(['{owner}', '{repo}'], [$this->owner, $this->repo], '/repos/{owner}/{repo}/actions/oidc/customization/sub'), ['Content-Type' => 'application/json'], json_encode($data));
    }
