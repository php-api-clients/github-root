...
final readonly class ListWorkflowRunsForRepo
{
    ...
    /** @return */
    public function call(string $owner, string $repo, string $actor, string $branch, string $event, string $status, string $created, int $checkSuiteId, int $perPage = 30, int $page = 1, bool $excludePullRequests = false): Ok
    {
        $operation = new \ApiClients\Client\GitHubEnterprise\Internal\Operation\Actions\ListWorkflowRunsForRepo($this->responseSchemaValidator, $this->hydrator, $owner, $repo, $actor, $branch, $event, $status, $created, $checkSuiteId, $perPage, $page, $excludePullRequests);
