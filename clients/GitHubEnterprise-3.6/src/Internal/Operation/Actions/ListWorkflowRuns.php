...
final class ListWorkflowRuns
{
    ...
    public function __construct(private readonly SchemaValidator $responseSchemaValidator, private readonly Internal\Hydrator\Operation\Repos\Owner\Repo\Actions\Workflows\WorkflowId\Runs $hydrator, string $owner, string $repo, $workflowId, string $actor, string $branch, string $event, string $status, string $created, int $checkSuiteId, int $perPage = 30, int $page = 1, bool $excludePullRequests = false)
    {
        $this->owner               = $owner;
        $this->repo                = $repo;
    ...
    {
        return new Request('GET', str_replace(['{owner}', '{repo}', '{workflow_id}', '{actor}', '{branch}', '{event}', '{status}', '{created}', '{check_suite_id}', '{per_page}', '{page}', '{exclude_pull_requests}'], [$this->owner, $this->repo, $this->workflowId, $this->actor, $this->branch, $this->event, $this->status, $this->created, $this->checkSuiteId, $this->perPage, $this->page, $this->excludePullRequests], '/repos/{owner}/{repo}/actions/workflows/{workflow_id}/runs' . '?actor={actor}&branch={branch}&event={event}&status={status}&created={created}&check_suite_id={check_suite_id}&per_page={per_page}&page={page}&exclude_pull_requests={exclude_pull_requests}'));
    }
