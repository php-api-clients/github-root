...
final class Six
{
    ...
    /** @return Schema\PreReceiveEnvironmentDownloadStatus|Schema\ActionsCacheUsageOrgEnterprise|Schema\ActionsCacheUsagePolicyEnterprise|Schema\Operations\EnterpriseAdmin\ListSelectedOrganizationsEnabledGithubActionsEnterprise\Response\ApplicationJson\Ok|Schema\SelectedActions|Schema\ActionsGetDefaultWorkflowPermissions|Schema\RunnerGroupsEnterprise|iterable<int,Schema\RunnerApplication>|Schema\Runner|Schema\AdvancedSecurityActiveCommitters|Schema\Operations\EnterpriseAdmin\GetManageConfigApplyEvents\Response\ApplicationJson\Ok|WithoutBody|Schema\GhesLicenseCheck|Schema\Operations\Actions\GetActionsCacheUsageByRepoForOrg\Response\ApplicationJson\Ok|Schema\Operations\Actions\ListSelectedRepositoriesEnabledGithubActionsOrganization\Response\ApplicationJson\Ok|Schema\RunnerGroupsOrg|Schema\ActionsPublicKey|Schema\OrganizationActionsSecret|Schema\OrganizationActionsVariable|Schema\DependabotPublicKey|Schema\OrganizationDependabotSecret|Schema\WebhookConfig|iterable<int,Schema\HookDeliveryItem>|iterable<int,Schema\MinimalRepository>|iterable<int,Schema\TeamRoleAssignment>|iterable<int,Schema\UserRoleAssignment>|Schema\Package|Schema\CustomProperty|Schema\RuleSuite|iterable<int,Schema\TeamDiscussion>|Schema\ExternalGroups|iterable<int,Schema\SimpleUser>|iterable<int,Schema\TeamProject>|iterable<int,Schema\Team>|Schema\ProjectCollaboratorPermission|Schema\Operations\Actions\ListArtifactsForRepo\Response\ApplicationJson\Ok|Schema\ActionsCacheList|Schema\Operations\Actions\ListRepoOrganizationSecrets\Response\ApplicationJson\Ok|Schema\Operations\Actions\ListRepoOrganizationVariables\Response\ApplicationJson\Ok|Schema\ActionsRepositoryPermissions|Schema\Operations\Actions\ListSelfHostedRunnersForRepo\Response\ApplicationJson\Ok\Application\Json|Schema\Operations\Actions\ListWorkflowRunsForRepo\Response\ApplicationJson\Ok|Schema\Operations\Actions\ListRepoSecrets\Response\ApplicationJson\Ok\Application\Json|Schema\Operations\Actions\ListRepoVariables\Response\ApplicationJson\Ok\Application\Json|Schema\Operations\Actions\ListRepoWorkflows\Response\ApplicationJson\Ok|Schema\Autolink|Schema\BranchWithProtection|Schema\BasicError|Schema\CheckRun|Schema\CheckSuite|iterable<int,Schema\CodeScanningAlertItems>|iterable<int,Schema\CodeScanningAnalysis>|Schema\CodeScanningDefaultSetup|Schema\CodeownersErrors|Schema\CommitComment|Schema\Commit|Schema\CommitComparison|Schema\ContentDirectory|Schema\ContentFile|Schema\ContentSymlink|Schema\ContentSubmodule|iterable<int,Schema\DependabotAlert>|Schema\Operations\Dependabot\ListRepoSecrets\Response\ApplicationJson\Ok|Schema\DependencyGraphSpdxSbom|Schema\Deployment|Schema\Environment|Schema\Hook|iterable<int,Schema\IssueComment>|iterable<int,Schema\IssueEvent>|Schema\Issue|Schema\DeployKey|Schema\Label|Schema\Milestone|iterable<int,Schema\PageBuild>|Schema\RepositoryPreReceiveHook|iterable<int,Schema\CustomPropertyValue>|iterable<int,Schema\PullRequestReviewComment>|Schema\PullRequest|Schema\Release|iterable<int,Schema\Operations\Repos\ListCacheInfo\Response\ApplicationJson\Ok>|iterable<int,Schema\RuleSuites>|Schema\RepositoryRuleset|iterable<int,Schema\SecretScanningAlert>|iterable<int,int>|Schema\Operations\Repos\GetCodeFrequencyStats\Response\ApplicationJson\Accepted\Application\Json|iterable<int,Schema\CommitActivity>|Schema\Operations\Repos\GetCommitActivityStats\Response\ApplicationJson\Accepted\Application\Json|iterable<int,Schema\ContributorActivity>|Schema\Operations\Repos\GetContributorsStats\Response\ApplicationJson\Accepted\Application\Json|Schema\ParticipationStats|iterable<int,Schema\TagProtection>|Schema\ScimEnterpriseGroupList|Schema\ScimEnterpriseUserList|iterable<int,Schema\TeamDiscussionComment>|iterable<int,Schema\Reaction>|Schema\TeamRepository|iterable<int,Schema\PackageVersion>|iterable<int,Schema\Event> */
                ...
                if ($pathChunks[2] === 'v1') {
                    if ($pathChunks[3] === 'config') {
                        if ($pathChunks[4] === 'apply') {
                            if ($pathChunks[5] === 'events') {
                                if ($call === 'GET /manage/v1/config/apply/events') {
                                    return $this->routers->internal🔀Router🔀Get🔀EnterpriseAdmin()->getManageConfigApplyEvents($params);
                                }
                            }
                        } elseif ($pathChunks[4] === 'license') {
                            if ($pathChunks[5] === 'check') {
                                if ($call === 'GET /manage/v1/config/license/check') {
                                    return $this->routers->internal🔀Router🔀Get🔀EnterpriseAdmin()->licenseCheck($params);
                                }
                            }
                        }
                    }
