...
final class ListGrantsListing
{
    ...
    public function __construct(private readonly SchemaValidator $responseSchemaValidator, private readonly Internal\Hydrator\Operation\Applications\Grants $hydrator, string $clientId, int $perPage = 30, int $page = 1)
    {
    ...
    {
        return new Request('GET', str_replace(['{client_id}', '{per_page}', '{page}'], [$this->clientId, $this->perPage, $this->page], '/applications/grants' . '?client_id={client_id}&per_page={per_page}&page={page}'));
    }
        ...
        switch ($contentType) {
            case 'application/json':
                ...
                switch ($code) {
                     ...
                     **/
                    case 200:
                        return Observable::fromArray($body, new ImmediateScheduler())->map(function (array $body): Schema\ApplicationGrant {
