...
/** @covers \ApiClients\Client\GitHubEnterprise\Internal\Operation\Orgs\UpdatePatAccesses */
final class UpdatePatAccessesTest extends AsyncTestCase
{
    ...
    public function call_httpCode_500_requestContentType_application_json_responseContentType_application_json_zero(): void
    {
        ...
        $browser->request('POST', '/orgs/generated/personal-access-tokens', Argument::type('array'), json_encode(json_decode(Schema\Orgs\UpdatePatAccesses\Request\ApplicationJson::SCHEMA_EXAMPLE_DATA, true)))->willReturn(resolve($response))->shouldBeCalled();
        $client = new Client($auth->reveal(), $browser->reveal());
        $result = $client->call(Internal\Operation\Orgs\UpdatePatAccesses::OPERATION_MATCH, (static function (array $data): array {
            $data['org'] = 'generated';
    ...
    public function operations_httpCode_500_requestContentType_application_json_responseContentType_application_json_zero(): void
    {
        ...
        $browser->request('POST', '/orgs/generated/personal-access-tokens', Argument::type('array'), json_encode(json_decode(Schema\Orgs\UpdatePatAccesses\Request\ApplicationJson::SCHEMA_EXAMPLE_DATA, true)))->willReturn(resolve($response))->shouldBeCalled();
        $client = new Client($auth->reveal(), $browser->reveal());
        $result = $client->operations()->orgs()->updatePatAccesses('generated', json_decode(Schema\Orgs\UpdatePatAccesses\Request\ApplicationJson::SCHEMA_EXAMPLE_DATA, true));
    }
    ...
    {
        $response = new Response(202, ['Content-Type' => 'application/json'], json_encode(json_decode(Schema\Operations\Orgs\UpdatePatAccesses\Response\ApplicationJson\Accepted\Application\Json::SCHEMA_EXAMPLE_DATA, true)));
        $auth     = $this->prophesize(AuthenticationInterface::class);
        $auth->authHeader(Argument::any())->willReturn('Bearer beer')->shouldBeCalled();
        $browser = $this->prophesize(Browser::class);
        $browser->withBase(Argument::any())->willReturn($browser->reveal());
