...
final class Five
{
    ...
    /** @return WithoutBody|Schema\PreReceiveEnvironmentDownloadStatus|Schema\Authorization|Schema\InstallationToken|Schema\RunnerGroupsEnterprise|iterable<int,Schema\GhesSetSshResponse>|Schema\Operations\EnterpriseAdmin\PostManageRunConfigApply\Response\ApplicationJson\Ok|Schema\RunnerGroupsOrg|Schema\EmptyObject|Schema\CodeSecurityConfiguration|Schema\ProjectCard|Schema\Operations\Projects\MoveColumn\Response\ApplicationJson\Created\Application\Json|Schema\Autolink|Schema\CheckRun|Schema\CheckSuite|Schema\Deployment|Schema\Operations\Repos\CreateDeployment\Response\ApplicationJson\Accepted\Application\Json|Schema\FullRepository|Schema\Hook|Schema\Issue|Schema\DeployKey|Schema\Label|Schema\MergedUpstream|Schema\Commit|Schema\Milestone|Schema\Page|Schema\Project|Schema\PullRequest|Schema\Release|Schema\RepositoryRuleset|Schema\MinimalRepository */
    public function call(string $call, array $params, array $pathChunks): WithoutBody|PreReceiveEnvironmentDownloadStatus|Authorization|InstallationToken|RunnerGroupsEnterprise|iterable|Ok|RunnerGroupsOrg|EmptyObject|CodeSecurityConfiguration|ProjectCard|Json|Autolink|CheckRun|CheckSuite|Deployment|\ApiClients\Client\GitHubEnterprise\Schema\Operations\Repos\CreateDeployment\Response\ApplicationJson\Accepted\Application\Json|FullRepository|Hook|Issue|DeployKey|Label|MergedUpstream|Commit|Milestone|Page|Project|PullRequest|Release|RepositoryRuleset|MinimalRepository
    {
        if ($pathChunks[0] === '') {
                    ...
                    if ($pathChunks[3] === 'access') {
                        ...
                        if ($pathChunks[4] === 'apply') {
                            if ($call === 'POST /manage/v1/config/apply') {
                                return $this->routers->internal🔀Router🔀Post🔀EnterpriseAdmin()->postManageRunConfigApply($params);
                            }
                        } elseif ($pathChunks[4] === 'init') {
