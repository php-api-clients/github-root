...
namespace ApiClients\Client\GitHubEnterprise\Internal\Hydrator\Operation\Manage\V1\Replication;
...
class Status implements ObjectMapper
{
    ...
    private function hydrateApiClients⚡️Client⚡️GitHubEnterprise⚡️Schema⚡️GhesReplicationStatus(array $payload): GhesReplicationStatus
    {
        $properties    = [];
        $missingFields = [];
        try {
        ...
        if (count($missingFields) > 0) {
            throw UnableToHydrateObject::dueToMissingFields(GhesReplicationStatus::class, $missingFields, stack: $this->hydrationStack);
        }

        try {
            return new GhesReplicationStatus(...$properties);
        } catch (Throwable $exception) {
            throw UnableToHydrateObject::dueToError('ApiClients\Client\GitHubEnterprise\Schema\GhesReplicationStatus', $exception, stack: $this->hydrationStack);
        }
    }
    ...
    {
        assert($object instanceof GhesReplicationStatus);
        $result = [];
