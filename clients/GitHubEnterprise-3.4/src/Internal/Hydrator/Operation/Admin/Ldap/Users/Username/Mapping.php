...
namespace ApiClients\Client\GitHubEnterprise\Internal\Hydrator\Operation\Admin\Ldap\Users\Username;

use ApiClients\Client\GitHubEnterprise\Schema\LdapMappingUser;
use ApiClients\Client\GitHubEnterprise\Schema\LdapMappingUser\Plan;
...
class Mapping implements ObjectMapper
{
    ...
    private function hydrateApiClients⚡️Client⚡️GitHubEnterprise⚡️Schema⚡️LdapMappingUser(array $payload): LdapMappingUser
    {
        $properties    = [];
        $missingFields = [];
        try {
        ...
        if (count($missingFields) > 0) {
            throw UnableToHydrateObject::dueToMissingFields(LdapMappingUser::class, $missingFields, stack: $this->hydrationStack);
        }

        try {
            return new LdapMappingUser(...$properties);
        } catch (Throwable $exception) {
            throw UnableToHydrateObject::dueToError('ApiClients\Client\GitHubEnterprise\Schema\LdapMappingUser', $exception, stack: $this->hydrationStack);
        }
    }

    private function hydrateApiClients⚡️Client⚡️GitHubEnterprise⚡️Schema⚡️LdapMappingUser⚡️Plan(array $payload): Plan
    {
        $properties    = [];
        $missingFields = [];
        try {
    ...
    {
        assert($object instanceof LdapMappingUser);
        $result = [];
