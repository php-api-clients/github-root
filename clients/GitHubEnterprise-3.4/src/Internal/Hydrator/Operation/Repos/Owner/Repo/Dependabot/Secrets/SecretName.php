...
class SecretName implements ObjectMapper
{
    ...
    private function hydrateApiClients⚡️Client⚡️GitHubEnterprise⚡️Schema⚡️DependabotSecret(array $payload): DependabotSecret
    {
        $properties    = [];
        $missingFields = [];
        try {
        ...
        if (count($missingFields) > 0) {
            throw UnableToHydrateObject::dueToMissingFields(DependabotSecret::class, $missingFields, stack: $this->hydrationStack);
        }

        try {
            return new DependabotSecret(...$properties);
        } catch (Throwable $exception) {
            throw UnableToHydrateObject::dueToError('ApiClients\Client\GitHubEnterprise\Schema\DependabotSecret', $exception, stack: $this->hydrationStack);
        }
    }
    ...
    {
        assert($object instanceof DependabotSecret);
        $result = [];
