...
final class ListAlertsForOrg
{
    ...
    public function __construct(private readonly SchemaValidator $responseSchemaValidator, private readonly Internal\Hydrator\Operation\Orgs\Org\SecretScanning\Alerts $hydrator, string $org, string $state, string $secretType, string $resolution, int $page = 1, int $perPage = 30)
    {
        $this->org        = $org;
        $this->state      = $state;
    ...
    {
        return new Request('GET', str_replace(['{org}', '{state}', '{secret_type}', '{resolution}', '{page}', '{per_page}'], [$this->org, $this->state, $this->secretType, $this->resolution, $this->page, $this->perPage], '/orgs/{org}/secret-scanning/alerts' . '?state={state}&secret_type={secret_type}&resolution={resolution}&page={page}&per_page={per_page}'));
    }
