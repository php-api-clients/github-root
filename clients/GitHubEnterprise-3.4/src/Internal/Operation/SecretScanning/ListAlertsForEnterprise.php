...
final class ListAlertsForEnterprise
{
    ...
    public function __construct(private readonly SchemaValidator $responseSchemaValidator, private readonly Internal\Hydrator\Operation\Enterprises\Enterprise\SecretScanning\Alerts $hydrator, string $enterprise, string $state, string $secretType, string $resolution, string $before, string $after, int $perPage = 30)
    {
    ...
    {
        return new Request('GET', str_replace(['{enterprise}', '{state}', '{secret_type}', '{resolution}', '{before}', '{after}', '{per_page}'], [$this->enterprise, $this->state, $this->secretType, $this->resolution, $this->before, $this->after, $this->perPage], '/enterprises/{enterprise}/secret-scanning/alerts' . '?state={state}&secret_type={secret_type}&resolution={resolution}&before={before}&after={after}&per_page={per_page}'));
    }
