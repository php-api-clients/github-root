...
use ApiClients\Client\GitHub\Internal\Operation\Apps\ListSubscriptionsForAuthenticatedUser;
...
/** @covers \ApiClients\Client\GitHub\Internal\Operation\Apps\ListSubscriptionsForAuthenticatedUser */
final class ListSubscriptionsForAuthenticatedUserTest extends AsyncTestCase
{
    /** @test */
    public function call_httpCode_401_responseContentType_application_json_zero(): void
    {
        ...
        $browser->request('GET', '/user/marketplace_purchases?page=1&per_page=8', Argument::type('array'), Argument::any())->willReturn(resolve($response))->shouldBeCalled();
        $client = new Client($auth->reveal(), $browser->reveal());
        $result = $client->call(ListSubscriptionsForAuthenticatedUser::OPERATION_MATCH, (static function (array $data): array {
            $data['per_page'] = 8;
            $data['page']     = 1;
    ...
    public function operations_httpCode_401_responseContentType_application_json_zero(): void
    {
        ...
        $browser->request('GET', '/user/marketplace_purchases?page=1&per_page=8', Argument::type('array'), Argument::any())->willReturn(resolve($response))->shouldBeCalled();
        $client = new Client($auth->reveal(), $browser->reveal());
        $result = $client->operations()->apps()->listSubscriptionsForAuthenticatedUser(8, 1);
    }
