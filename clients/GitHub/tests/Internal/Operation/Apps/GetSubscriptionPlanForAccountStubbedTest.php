...
use ApiClients\Client\GitHub\Internal\Operation\Apps\GetSubscriptionPlanForAccountStubbed;
...
/** @covers \ApiClients\Client\GitHub\Internal\Operation\Apps\GetSubscriptionPlanForAccountStubbed */
final class GetSubscriptionPlanForAccountStubbedTest extends AsyncTestCase
{
    /** @test */
    public function call_httpCode_200_responseContentType_application_json_zero(): void
    {
        $response = new Response(200, ['Content-Type' => 'application/json'], json_encode(json_decode(MarketplacePurchase::SCHEMA_EXAMPLE_DATA, true)));
        $auth     = $this->prophesize(AuthenticationInterface::class);
        $auth->authHeader(Argument::any())->willReturn('Bearer beer')->shouldBeCalled();
        $browser = $this->prophesize(Browser::class);
        $browser->withBase(Argument::any())->willReturn($browser->reveal());
        $browser->withFollowRedirects(Argument::any())->willReturn($browser->reveal());
        $browser->request('GET', '/marketplace_listing/stubbed/accounts/10', Argument::type('array'), Argument::any())->willReturn(resolve($response))->shouldBeCalled();
        $client = new Client($auth->reveal(), $browser->reveal());
        $result = $client->call(GetSubscriptionPlanForAccountStubbed::OPERATION_MATCH, (static function (array $data): array {
            $data['account_id'] = 10;
    ...
    public function operations_httpCode_200_responseContentType_application_json_zero(): void
    {
        ...
        $browser->request('GET', '/marketplace_listing/stubbed/accounts/10', Argument::type('array'), Argument::any())->willReturn(resolve($response))->shouldBeCalled();
        $client = new Client($auth->reveal(), $browser->reveal());
        $result = $client->operations()->apps()->getSubscriptionPlanForAccountStubbed(10);
    }
