...
final class ListForOrg
{
    ...
    public function __construct(private SchemaValidator $responseSchemaValidator, private Projects $hydrator, string $org, string $state = 'open', int $perPage = 30, int $page = 1)
    {
        $this->org                     = $org;
        $this->state                   = $state;
    ...
    {
        return new Request('GET', (string) (new UriTemplate('/orgs/{org}/projects{?page,per_page,state}'))->expand(['org' => $this->org, 'page' => $this->page, 'per_page' => $this->perPage, 'state' => $this->state]));
    }
