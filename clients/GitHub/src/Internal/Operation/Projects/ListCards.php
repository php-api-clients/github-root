...
final class ListCards
{
    ...
    public function __construct(private SchemaValidator $responseSchemaValidator, private Cards $hydrator, int $columnId, string $archivedState = 'not_archived', int $perPage = 30, int $page = 1)
    {
    ...
    {
        return new Request('GET', (string) (new UriTemplate('/projects/columns/{column_id}/cards{?archived_state,page,per_page}'))->expand(['archived_state' => $this->archivedState, 'column_id' => $this->columnId, 'page' => $this->page, 'per_page' => $this->perPage]));
    }
        ...
        switch ($contentType) {
            case 'application/json':
                ...
                switch ($code) {
                     ...
                     **/
                    case 200:
                        return Observable::fromArray($body, new ImmediateScheduler())->map(function (array $body): ProjectCard {
                            $error = new RuntimeException();
                            try {
                                $this->responseSchemaValidator->validate($body, Reader::readFromJson(ProjectCard::SCHEMA_JSON, '\\cebe\\openapi\\spec\\Schema'));
