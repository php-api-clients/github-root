...
final class ListDeploymentBranchPolicies
{
    ...
    public function __construct(private SchemaValidator $responseSchemaValidator, private DeploymentBranchPolicies $hydrator, string $owner, string $repo, string $environmentName, int $perPage = 30, int $page = 1)
    {
        $this->owner                   = $owner;
        $this->repo                    = $repo;
    ...
    {
        return new Request('GET', (string) (new UriTemplate('/repos/{owner}/{repo}/environments/{environment_name}/deployment-branch-policies{?page,per_page}'))->expand(['environment_name' => $this->environmentName, 'owner' => $this->owner, 'page' => $this->page, 'per_page' => $this->perPage, 'repo' => $this->repo]));
    }
