...
use ApiClients\Client\GitHub\Schema\BranchShort;
...
final class ListBranchesForHeadCommit
{
    ...
    public function __construct(private SchemaValidator $responseSchemaValidator, private BranchesWhereHead $hydrator, string $owner, string $repo, string $commitSha)
    {
        $this->owner                   = $owner;
        $this->repo                    = $repo;
    ...
    {
        return new Request('GET', (string) (new UriTemplate('/repos/{owner}/{repo}/commits/{commit_sha}/branches-where-head'))->expand(['commit_sha' => $this->commitSha, 'owner' => $this->owner, 'repo' => $this->repo]));
    }
        ...
        switch ($contentType) {
            case 'application/json':
                ...
                switch ($code) {
                     ...
                     **/
                    case 200:
                        return Observable::fromArray($body, new ImmediateScheduler())->map(function (array $body): BranchShort {
                            $error = new RuntimeException();
                            try {
                                $this->responseSchemaValidator->validate($body, Reader::readFromJson(BranchShort::SCHEMA_JSON, '\\cebe\\openapi\\spec\\Schema'));

                                return $this->hydrator->hydrateObject(BranchShort::class, $body);
