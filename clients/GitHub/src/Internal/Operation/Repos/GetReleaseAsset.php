...
final class GetReleaseAsset
{
    ...
    public function __construct(private SchemaValidator $responseSchemaValidator, private AssetId $hydrator, string $owner, string $repo, int $assetId)
    {
        $this->owner                   = $owner;
        $this->repo                    = $repo;
    ...
    {
        return new Request('GET', (string) (new UriTemplate('/repos/{owner}/{repo}/releases/assets/{asset_id}'))->expand(['asset_id' => $this->assetId, 'owner' => $this->owner, 'repo' => $this->repo]));
    }
