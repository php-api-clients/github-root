...
final class ListForOrg
{
    ...
    public function __construct(private SchemaValidator $responseSchemaValidator, private Repos $hydrator, string $org, string $direction, string $type = 'all', string $sort = 'created', int $perPage = 30, int $page = 1)
    {
        $this->org                     = $org;
    ...
    {
        return new Request('GET', (string) (new UriTemplate('/orgs/{org}/repos{?direction,page,per_page,sort,type}'))->expand(['direction' => $this->direction, 'org' => $this->org, 'page' => $this->page, 'per_page' => $this->perPage, 'sort' => $this->sort, 'type' => $this->type]));
    }
