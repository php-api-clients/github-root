...
final class DeletePullRequestReviewProtection
{
    ...
    public function __construct(private SchemaValidator $responseSchemaValidator, private RequiredPullRequestReviews $hydrator, string $owner, string $repo, string $branch)
    {
        $this->owner                   = $owner;
        $this->repo                    = $repo;
        $this->branch                  = $branch;
    ...
    {
        return new Request('DELETE', (string) (new UriTemplate('/repos/{owner}/{repo}/branches/{branch}/protection/required_pull_request_reviews'))->expand(['branch' => $this->branch, 'owner' => $this->owner, 'repo' => $this->repo]));
    }
