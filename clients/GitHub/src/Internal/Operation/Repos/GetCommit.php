...
final class GetCommit
{
    ...
    public function __construct(private SchemaValidator $responseSchemaValidator, private Ref $hydrator, string $owner, string $repo, string $ref, int $page = 1, int $perPage = 30)
    {
        $this->owner                   = $owner;
        $this->repo                    = $repo;
        $this->ref                     = $ref;
    ...
    {
        return new Request('GET', (string) (new UriTemplate('/repos/{owner}/{repo}/commits/{ref}{?page,per_page}'))->expand(['owner' => $this->owner, 'page' => $this->page, 'per_page' => $this->perPage, 'ref' => $this->ref, 'repo' => $this->repo]));
    }

    public function createResponse(ResponseInterface $response): Commit
    {
        ...
        switch ($contentType) {
            case 'application/json':
                ...
                switch ($code) {
                     ...
                     **/
                    case 200:
                        $this->responseSchemaValidator->validate($body, Reader::readFromJson(Commit::SCHEMA_JSON, Schema::class));
