...
final class ListReleaseAssets
{
    ...
    public function __construct(private SchemaValidator $responseSchemaValidator, private Assets $hydrator, string $owner, string $repo, int $releaseId, int $perPage = 30, int $page = 1)
    {
        $this->owner                   = $owner;
        $this->repo                    = $repo;
    ...
    {
        return new Request('GET', (string) (new UriTemplate('/repos/{owner}/{repo}/releases/{release_id}/assets{?page,per_page}'))->expand(['owner' => $this->owner, 'page' => $this->page, 'per_page' => $this->perPage, 'release_id' => $this->releaseId, 'repo' => $this->repo]));
    }
