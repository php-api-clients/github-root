...
final class CancelWorkflowRun
{
    ...
    public function __construct(private SchemaValidator $responseSchemaValidator, private Cancel $hydrator, string $owner, string $repo, int $runId)
    {
        $this->owner                   = $owner;
        $this->repo                    = $repo;
        $this->runId                   = $runId;
    ...
    {
        return new Request('POST', (string) (new UriTemplate('/repos/{owner}/{repo}/actions/runs/{run_id}/cancel'))->expand(['owner' => $this->owner, 'repo' => $this->repo, 'run_id' => $this->runId]));
    }
