...
use ApiClients\Client\GitHub\Internal\Hydrator\Operation\Repos\Owner\Repo\Actions\Variables\Name;
...
final class GetRepoVariable
{
    ...
    public function __construct(private SchemaValidator $responseSchemaValidator, private Name $hydrator, string $owner, string $repo, string $name)
    {
        $this->owner                   = $owner;
        $this->repo                    = $repo;
        $this->name                    = $name;
    ...
    {
        return new Request('GET', (string) (new UriTemplate('/repos/{owner}/{repo}/actions/variables/{name}'))->expand(['name' => $this->name, 'owner' => $this->owner, 'repo' => $this->repo]));
    }
