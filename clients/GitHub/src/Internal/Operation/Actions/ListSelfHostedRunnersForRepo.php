...
use ApiClients\Client\GitHub\Internal\Hydrator\Operation\Repos\Owner\Repo\Actions\Runners;
...
final class ListSelfHostedRunnersForRepo
{
    ...
    public function __construct(private SchemaValidator $responseSchemaValidator, private Runners $hydrator, string $name, string $owner, string $repo, int $perPage = 30, int $page = 1)
    {
        $this->name                    = $name;
        $this->owner                   = $owner;
        $this->repo                    = $repo;
    ...
    {
        return new Request('GET', (string) (new UriTemplate('/repos/{owner}/{repo}/actions/runners{?name,page,per_page}'))->expand(['name' => $this->name, 'owner' => $this->owner, 'page' => $this->page, 'per_page' => $this->perPage, 'repo' => $this->repo]));
    }
