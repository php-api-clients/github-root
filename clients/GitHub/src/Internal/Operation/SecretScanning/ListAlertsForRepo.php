...
use ApiClients\Client\GitHub\Internal\Hydrator\Operation\Repos\Owner\Repo\SecretScanning\Alerts;
...
final class ListAlertsForRepo
{
    ...
    public function __construct(private SchemaValidator $responseSchemaValidator, private Alerts $hydrator, string $owner, string $repo, string $state, string $secretType, string $resolution, string $before, string $after, string $validity, string $sort = 'created', string $direction = 'desc', int $page = 1, int $perPage = 30, bool $isPubliclyLeaked = false, bool $isMultiRepo = false)
    {
        $this->owner                   = $owner;
        $this->repo                    = $repo;
        $this->state                   = $state;
    ...
    {
        return new Request('GET', (string) (new UriTemplate('/repos/{owner}/{repo}/secret-scanning/alerts{?after,before,direction,is_multi_repo,is_publicly_leaked,page,per_page,resolution,secret_type,sort,state,validity}'))->expand(['after' => $this->after, 'before' => $this->before, 'direction' => $this->direction, 'is_multi_repo' => $this->isMultiRepo, 'is_publicly_leaked' => $this->isPubliclyLeaked, 'owner' => $this->owner, 'page' => $this->page, 'per_page' => $this->perPage, 'repo' => $this->repo, 'resolution' => $this->resolution, 'secret_type' => $this->secretType, 'sort' => $this->sort, 'state' => $this->state, 'validity' => $this->validity]));
    }
        ...
        switch ($contentType) {
            case 'application/json':
                ...
                switch ($code) {
                     ...
                     **/
                    case 200:
                        return Observable::fromArray($body, new ImmediateScheduler())->map(function (array $body): SecretScanningAlert {
                            $error = new RuntimeException();
                            try {
                                $this->responseSchemaValidator->validate($body, Reader::readFromJson(SecretScanningAlert::SCHEMA_JSON, '\\cebe\\openapi\\spec\\Schema'));
