...
final class GetBlob
{
    ...
    public function __construct(private SchemaValidator $responseSchemaValidator, private FileSha $hydrator, string $owner, string $repo, private string $fileSha)
    {
        $this->owner                   = $owner;
        $this->repo                    = $repo;
    ...
    {
        return new Request('GET', (string) (new UriTemplate('/repos/{owner}/{repo}/git/blobs/{file_sha}'))->expand(['file_sha' => $this->fileSha, 'owner' => $this->owner, 'repo' => $this->repo]));
    }

    public function createResponse(ResponseInterface $response): Blob
    {
        ...
        switch ($contentType) {
            case 'application/json':
                ...
                switch ($code) {
                     ...
                     **/
                    case 200:
                        $this->responseSchemaValidator->validate($body, Reader::readFromJson(Blob::SCHEMA_JSON, Schema::class));

                        return $this->hydrator->hydrateObject(Blob::class, $body);
                    /**
                     * Resource not found
                     **/
