...
use ApiClients\Client\GitHub\Internal\Hydrator\Operation\Repos\Owner\Repo\Pulls\Comments;
...
final class ListReviewCommentsForRepo
{
    ...
    public function __construct(private SchemaValidator $responseSchemaValidator, private Comments $hydrator, string $owner, string $repo, private string $sort, string $direction, string $since, int $perPage = 30, int $page = 1)
    {
        $this->owner                   = $owner;
        $this->repo                    = $repo;
    ...
    {
        return new Request('GET', (string) (new UriTemplate('/repos/{owner}/{repo}/pulls/comments{?direction,page,per_page,since,sort}'))->expand(['direction' => $this->direction, 'owner' => $this->owner, 'page' => $this->page, 'per_page' => $this->perPage, 'repo' => $this->repo, 'since' => $this->since, 'sort' => $this->sort]));
    }
