...
final class ListRepoNotificationsForAuthenticatedUser
{
    ...
    public function __construct(private SchemaValidator $responseSchemaValidator, private Notifications $hydrator, string $owner, string $repo, string $since, string $before, bool $all = false, bool $participating = false, int $perPage = 30, int $page = 1)
    {
        $this->owner                   = $owner;
        $this->repo                    = $repo;
        $this->since                   = $since;
    ...
    {
        return new Request('GET', (string) (new UriTemplate('/repos/{owner}/{repo}/notifications{?all,before,page,participating,per_page,since}'))->expand(['all' => $this->all, 'before' => $this->before, 'owner' => $this->owner, 'page' => $this->page, 'participating' => $this->participating, 'per_page' => $this->perPage, 'repo' => $this->repo, 'since' => $this->since]));
    }
