...
final class ListWatchersForRepo
{
    ...
    public function __construct(private SchemaValidator $responseSchemaValidator, private Subscribers $hydrator, string $owner, string $repo, int $perPage = 30, int $page = 1)
    {
        $this->owner                   = $owner;
        $this->repo                    = $repo;
    ...
    {
        return new Request('GET', (string) (new UriTemplate('/repos/{owner}/{repo}/subscribers{?page,per_page}'))->expand(['owner' => $this->owner, 'page' => $this->page, 'per_page' => $this->perPage, 'repo' => $this->repo]));
    }
