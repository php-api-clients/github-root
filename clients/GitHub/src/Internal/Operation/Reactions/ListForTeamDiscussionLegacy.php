...
final class ListForTeamDiscussionLegacy
{
    ...
    public function __construct(private SchemaValidator $responseSchemaValidator, private Reactions $hydrator, int $teamId, int $discussionNumber, string $content, int $perPage = 30, int $page = 1)
    {
        $this->teamId                  = $teamId;
    ...
    {
        return new Request('GET', (string) (new UriTemplate('/teams/{team_id}/discussions/{discussion_number}/reactions{?content,page,per_page}'))->expand(['content' => $this->content, 'discussion_number' => $this->discussionNumber, 'page' => $this->page, 'per_page' => $this->perPage, 'team_id' => $this->teamId]));
    }
