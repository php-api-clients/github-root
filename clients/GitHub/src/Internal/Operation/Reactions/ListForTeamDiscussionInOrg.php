...
final class ListForTeamDiscussionInOrg
{
    ...
    public function __construct(private SchemaValidator $responseSchemaValidator, private Reactions $hydrator, string $org, string $teamSlug, int $discussionNumber, string $content, int $perPage = 30, int $page = 1)
    {
        $this->org                     = $org;
    ...
    {
        return new Request('GET', (string) (new UriTemplate('/orgs/{org}/teams/{team_slug}/discussions/{discussion_number}/reactions{?content,page,per_page}'))->expand(['content' => $this->content, 'discussion_number' => $this->discussionNumber, 'org' => $this->org, 'page' => $this->page, 'per_page' => $this->perPage, 'team_slug' => $this->teamSlug]));
    }
