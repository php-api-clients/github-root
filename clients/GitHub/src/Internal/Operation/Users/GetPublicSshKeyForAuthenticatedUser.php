...
final class GetPublicSshKeyForAuthenticatedUser
{
    ...
    public function __construct(private SchemaValidator $responseSchemaValidator, private KeyId $hydrator, int $keyId)
    {
        $this->keyId                   = $keyId;
    ...
    {
        return new Request('GET', (string) (new UriTemplate('/user/keys/{key_id}'))->expand(['key_id' => $this->keyId]));
    }

    public function createResponse(ResponseInterface $response): Key|WithoutBody
    {
        ...
        switch ($contentType) {
            case 'application/json':
                ...
                switch ($code) {
                     ...
                     **/
                    case 200:
                        $this->responseSchemaValidator->validate($body, Reader::readFromJson(Key::SCHEMA_JSON, Schema::class));

                        return $this->hydrator->hydrateObject(Key::class, $body);
                    /**
                     * Resource not found
                     **/
