...
final class ListSshSigningKeysForAuthenticatedUser
{
    ...
    public function __construct(private SchemaValidator $responseSchemaValidator, private SshSigningKeys $hydrator, int $perPage = 30, int $page = 1)
    {
    ...
    {
        return new Request('GET', (string) (new UriTemplate('/user/ssh_signing_keys{?page,per_page}'))->expand(['page' => $this->page, 'per_page' => $this->perPage]));
    }
        ...
        switch ($contentType) {
            case 'application/json':
                ...
                switch ($code) {
                     ...
                     **/
                    case 200:
                        return Observable::fromArray($body, new ImmediateScheduler())->map(function (array $body): SshSigningKey {
                            $error = new RuntimeException();
                            try {
                                $this->responseSchemaValidator->validate($body, Reader::readFromJson(SshSigningKey::SCHEMA_JSON, '\\cebe\\openapi\\spec\\Schema'));
