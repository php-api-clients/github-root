...
use ApiClients\Client\GitHub\Internal\Hydrator\Operation\Repos\Owner\Repo\Milestones\MilestoneNumber\Labels;
...
final class ListLabelsForMilestone
{
    ...
    public function __construct(private SchemaValidator $responseSchemaValidator, private Labels $hydrator, string $owner, string $repo, int $milestoneNumber, int $perPage = 30, int $page = 1)
    {
        $this->owner                   = $owner;
        $this->repo                    = $repo;
    ...
    {
        return new Request('GET', (string) (new UriTemplate('/repos/{owner}/{repo}/milestones/{milestone_number}/labels{?page,per_page}'))->expand(['milestone_number' => $this->milestoneNumber, 'owner' => $this->owner, 'page' => $this->page, 'per_page' => $this->perPage, 'repo' => $this->repo]));
    }
