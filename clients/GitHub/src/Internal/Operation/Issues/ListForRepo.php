...
use ApiClients\Client\GitHub\Internal\Hydrator\Operation\Repos\Owner\Repo\Issues;
...
final class ListForRepo
{
    ...
    public function __construct(private SchemaValidator $responseSchemaValidator, private Issues $hydrator, string $owner, string $repo, string $milestone, string $assignee, string $creator, string $mentioned, string $labels, string $since, string $state = 'open', string $sort = 'created', string $direction = 'desc', int $perPage = 30, int $page = 1)
    {
        $this->owner                   = $owner;
        $this->repo                    = $repo;
    ...
    {
        return new Request('GET', (string) (new UriTemplate('/repos/{owner}/{repo}/issues{?assignee,creator,direction,labels,mentioned,milestone,page,per_page,since,sort,state}'))->expand(['assignee' => $this->assignee, 'creator' => $this->creator, 'direction' => $this->direction, 'labels' => $this->labels, 'mentioned' => $this->mentioned, 'milestone' => $this->milestone, 'owner' => $this->owner, 'page' => $this->page, 'per_page' => $this->perPage, 'repo' => $this->repo, 'since' => $this->since, 'sort' => $this->sort, 'state' => $this->state]));
    }
