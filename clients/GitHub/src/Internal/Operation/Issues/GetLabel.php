...
final class GetLabel
{
    ...
    public function __construct(private SchemaValidator $responseSchemaValidator, private Name $hydrator, string $owner, string $repo, private string $name)
    {
        $this->owner                   = $owner;
        $this->repo                    = $repo;
    ...
    {
        return new Request('GET', (string) (new UriTemplate('/repos/{owner}/{repo}/labels/{name}'))->expand(['name' => $this->name, 'owner' => $this->owner, 'repo' => $this->repo]));
    }
