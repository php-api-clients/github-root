...
final class List_
{
    ...
    public function __construct(private SchemaValidator $responseSchemaValidator, private Issues $hydrator, string $labels, string $since, private bool $collab, private bool $orgs, private bool $owned, private bool $pulls, string $filter = 'assigned', string $state = 'open', string $sort = 'created', string $direction = 'desc', int $perPage = 30, int $page = 1)
    {
        $this->labels                  = $labels;
        $this->since                   = $since;
        $this->filter                  = $filter;
    ...
    {
        return new Request('GET', (string) (new UriTemplate('/issues{?collab,direction,filter,labels,orgs,owned,page,per_page,pulls,since,sort,state}'))->expand(['collab' => $this->collab, 'direction' => $this->direction, 'filter' => $this->filter, 'labels' => $this->labels, 'orgs' => $this->orgs, 'owned' => $this->owned, 'page' => $this->page, 'per_page' => $this->perPage, 'pulls' => $this->pulls, 'since' => $this->since, 'sort' => $this->sort, 'state' => $this->state]));
    }
