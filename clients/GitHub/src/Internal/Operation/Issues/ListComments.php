...
use ApiClients\Client\GitHub\Internal\Hydrator\Operation\Repos\Owner\Repo\Issues\IssueNumber\Comments;
...
final class ListComments
{
    ...
    public function __construct(private SchemaValidator $responseSchemaValidator, private Comments $hydrator, string $owner, string $repo, int $issueNumber, string $since, int $perPage = 30, int $page = 1)
    {
        $this->owner                   = $owner;
        $this->repo                    = $repo;
    ...
    {
        return new Request('GET', (string) (new UriTemplate('/repos/{owner}/{repo}/issues/{issue_number}/comments{?page,per_page,since}'))->expand(['issue_number' => $this->issueNumber, 'owner' => $this->owner, 'page' => $this->page, 'per_page' => $this->perPage, 'repo' => $this->repo, 'since' => $this->since]));
    }
