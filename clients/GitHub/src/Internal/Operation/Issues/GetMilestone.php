...
final class GetMilestone
{
    ...
    public function __construct(private SchemaValidator $responseSchemaValidator, private MilestoneNumber $hydrator, string $owner, string $repo, int $milestoneNumber)
    {
        $this->owner                   = $owner;
        $this->repo                    = $repo;
    ...
    {
        return new Request('GET', (string) (new UriTemplate('/repos/{owner}/{repo}/milestones/{milestone_number}'))->expand(['milestone_number' => $this->milestoneNumber, 'owner' => $this->owner, 'repo' => $this->repo]));
    }
