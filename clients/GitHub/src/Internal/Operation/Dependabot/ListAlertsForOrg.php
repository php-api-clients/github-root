...
use ApiClients\Client\GitHub\Internal\Hydrator\Operation\Orgs\Org\Dependabot\Alerts;
...
final class ListAlertsForOrg
{
    ...
    public function __construct(private SchemaValidator $responseSchemaValidator, private Alerts $hydrator, string $org, string $state, string $severity, string $ecosystem, string $package, string $scope, string $before, string $after, int $last, string $sort = 'created', string $direction = 'desc', int $first = 30, int $perPage = 30)
    {
        $this->org                     = $org;
        $this->state                   = $state;
    ...
    {
        return new Request('GET', (string) (new UriTemplate('/orgs/{org}/dependabot/alerts{?after,before,direction,ecosystem,first,last,package,per_page,scope,severity,sort,state}'))->expand(['after' => $this->after, 'before' => $this->before, 'direction' => $this->direction, 'ecosystem' => $this->ecosystem, 'first' => $this->first, 'last' => $this->last, 'org' => $this->org, 'package' => $this->package, 'per_page' => $this->perPage, 'scope' => $this->scope, 'severity' => $this->severity, 'sort' => $this->sort, 'state' => $this->state]));
    }
