...
final class Code
{
    ...
    public function __construct(private SchemaValidator $responseSchemaValidator, private \ApiClients\Client\GitHub\Internal\Hydrator\Operation\Search\Code $hydrator, string $q, string $sort, string $order = 'desc', int $perPage = 30, int $page = 1)
    {
        $this->q                       = $q;
        $this->sort                    = $sort;
        $this->order                   = $order;
    ...
    {
        return new Request('GET', (string) (new UriTemplate('/search/code{?order,page,per_page,q,sort}'))->expand(['order' => $this->order, 'page' => $this->page, 'per_page' => $this->perPage, 'q' => $this->q, 'sort' => $this->sort]));
    }
