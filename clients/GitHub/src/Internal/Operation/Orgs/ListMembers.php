...
final class ListMembers
{
    ...
    public function __construct(private SchemaValidator $responseSchemaValidator, private Members $hydrator, string $org, string $filter = 'all', string $role = 'all', int $perPage = 30, int $page = 1)
    {
        $this->org                     = $org;
        $this->filter                  = $filter;
        $this->role                    = $role;
    ...
    {
        return new Request('GET', (string) (new UriTemplate('/orgs/{org}/members{?filter,page,per_page,role}'))->expand(['filter' => $this->filter, 'org' => $this->org, 'page' => $this->page, 'per_page' => $this->perPage, 'role' => $this->role]));
    }
