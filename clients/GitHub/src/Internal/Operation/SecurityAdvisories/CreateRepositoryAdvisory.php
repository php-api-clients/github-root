...
final class CreateRepositoryAdvisory
{
    ...
    public function __construct(private SchemaValidator $requestSchemaValidator, private SchemaValidator $responseSchemaValidator, private SecurityAdvisories $hydrator, string $owner, string $repo)
    {
    ...
    {
        $this->requestSchemaValidator->validate($data, Reader::readFromJson(RepositoryAdvisoryCreate::SCHEMA_JSON, Schema::class));

        return new Request('POST', (string) (new UriTemplate('/repos/{owner}/{repo}/security-advisories'))->expand(['owner' => $this->owner, 'repo' => $this->repo]), ['Content-Type' => 'application/json'], json_encode($data));
    }
