...
final class ListOrgRepositoryAdvisories
{
    ...
    public function __construct(private SchemaValidator $responseSchemaValidator, private SecurityAdvisories $hydrator, string $org, string $before, string $after, string $state, string $direction = 'desc', string $sort = 'created', int $perPage = 30)
    {
        $this->org                     = $org;
        $this->before                  = $before;
        $this->after                   = $after;
    ...
    {
        return new Request('GET', (string) (new UriTemplate('/orgs/{org}/security-advisories{?after,before,direction,per_page,sort,state}'))->expand(['after' => $this->after, 'before' => $this->before, 'direction' => $this->direction, 'org' => $this->org, 'per_page' => $this->perPage, 'sort' => $this->sort, 'state' => $this->state]));
    }
