...
use ApiClients\Client\GitHub\Internal\Hydrator\Operation\Repos\Owner\Repo\SecurityAdvisories\GhsaId\Forks;
...
{
    public const OPERATION_ID    = 'security-advisories/create-fork';
    public const OPERATION_MATCH = 'POST /repos/{owner}/{repo}/security-advisories/{ghsa_id}/forks';
    ...
    public function __construct(private SchemaValidator $responseSchemaValidator, private Forks $hydrator, string $owner, string $repo, string $ghsaId)
    {
        $this->owner                   = $owner;
        $this->repo                    = $repo;
        $this->ghsaId                  = $ghsaId;
    ...
    {
        return new Request('POST', (string) (new UriTemplate('/repos/{owner}/{repo}/security-advisories/{ghsa_id}/forks'))->expand(['ghsa_id' => $this->ghsaId, 'owner' => $this->owner, 'repo' => $this->repo]));
    }
