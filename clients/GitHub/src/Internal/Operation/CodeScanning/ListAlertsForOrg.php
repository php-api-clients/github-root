...
use ApiClients\Client\GitHub\Internal\Hydrator\Operation\Orgs\Org\CodeScanning\Alerts;
use ApiClients\Client\GitHub\Schema\BasicError;
use ApiClients\Client\GitHub\Schema\CodeScanningOrganizationAlertItems;
...
final class ListAlertsForOrg
{
    ...
    public function __construct(private SchemaValidator $responseSchemaValidator, private Alerts $hydrator, string $org, string $toolName, string|null $toolGuid, string $before, string $after, string $state, string $severity, int $page = 1, int $perPage = 30, string $direction = 'desc', string $sort = 'created')
    {
        $this->org                     = $org;
    ...
    {
        return new Request('GET', (string) (new UriTemplate('/orgs/{org}/code-scanning/alerts{?after,before,direction,page,per_page,severity,sort,state,tool_guid,tool_name}'))->expand(['after' => $this->after, 'before' => $this->before, 'direction' => $this->direction, 'org' => $this->org, 'page' => $this->page, 'per_page' => $this->perPage, 'severity' => $this->severity, 'sort' => $this->sort, 'state' => $this->state, 'tool_guid' => $this->toolGuid, 'tool_name' => $this->toolName]));
    }
                    ...
                    case 200:
                        return Observable::fromArray($body, new ImmediateScheduler())->map(function (array $body): CodeScanningOrganizationAlertItems {
                            ...
                            try {
                                ...
                                return $this->hydrator->hydrateObject(CodeScanningOrganizationAlertItems::class, $body);
