...
use ApiClients\Client\GitHub\Internal\Hydrator\Operation\Repos\Owner\Repo\CodeScanning\Alerts;
use ApiClients\Client\GitHub\Schema\BasicError;
use ApiClients\Client\GitHub\Schema\CodeScanningAlertItems;
...
final class ListAlertsForRepo
{
    ...
    public function __construct(private SchemaValidator $responseSchemaValidator, private Alerts $hydrator, string $owner, string $repo, string $toolName, string|null $toolGuid, string $ref, int $pr, string $before, string $after, string $state, string $severity, int $page = 1, int $perPage = 30, string $direction = 'desc', string $sort = 'created')
    {
        $this->owner                   = $owner;
        $this->repo                    = $repo;
    ...
    {
        return new Request('GET', (string) (new UriTemplate('/repos/{owner}/{repo}/code-scanning/alerts{?after,before,direction,page,per_page,pr,ref,severity,sort,state,tool_guid,tool_name}'))->expand(['after' => $this->after, 'before' => $this->before, 'direction' => $this->direction, 'owner' => $this->owner, 'page' => $this->page, 'per_page' => $this->perPage, 'pr' => $this->pr, 'ref' => $this->ref, 'repo' => $this->repo, 'severity' => $this->severity, 'sort' => $this->sort, 'state' => $this->state, 'tool_guid' => $this->toolGuid, 'tool_name' => $this->toolName]));
    }
                    ...
                    case 200:
                        return Observable::fromArray($body, new ImmediateScheduler())->map(function (array $body): CodeScanningAlertItems {
                            ...
                            try {
                                ...
                                return $this->hydrator->hydrateObject(CodeScanningAlertItems::class, $body);
