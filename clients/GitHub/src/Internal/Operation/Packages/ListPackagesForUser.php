...
use ApiClients\Client\GitHub\Internal\Hydrator\Operation\Users\Username\Packages;
...
final class ListPackagesForUser
{
    ...
    public function __construct(private SchemaValidator $responseSchemaValidator, private Packages $hydrator, string $packageType, string $visibility, string $username, int $page = 1, int $perPage = 30)
    {
    ...
    {
        return new Request('GET', (string) (new UriTemplate('/users/{username}/packages{?package_type,page,per_page,visibility}'))->expand(['package_type' => $this->packageType, 'page' => $this->page, 'per_page' => $this->perPage, 'username' => $this->username, 'visibility' => $this->visibility]));
    }
