...
final class GetAllPackageVersionsForPackageOwnedByAuthenticatedUser
{
    ...
    public function __construct(private SchemaValidator $responseSchemaValidator, private Versions $hydrator, string $packageType, string $packageName, int $page = 1, int $perPage = 30, string $state = 'active')
    {
    ...
    {
        return new Request('GET', (string) (new UriTemplate('/user/packages/{package_type}/{package_name}/versions{?page,per_page,state}'))->expand(['package_name' => $this->packageName, 'package_type' => $this->packageType, 'page' => $this->page, 'per_page' => $this->perPage, 'state' => $this->state]));
    }
