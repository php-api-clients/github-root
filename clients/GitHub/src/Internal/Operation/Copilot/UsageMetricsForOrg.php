...
use ApiClients\Client\GitHub\Internal\Hydrator\Operation\Orgs\Org\Copilot\Usage;
...
final class UsageMetricsForOrg
{
    ...
    public function __construct(private SchemaValidator $responseSchemaValidator, private Usage $hydrator, string $org, string $since, string $until, int $page = 1, int $perPage = 28)
    {
        $this->org                     = $org;
        $this->since                   = $since;
        $this->until                   = $until;
    ...
    {
        return new Request('GET', (string) (new UriTemplate('/orgs/{org}/copilot/usage{?page,per_page,since,until}'))->expand(['org' => $this->org, 'page' => $this->page, 'per_page' => $this->perPage, 'since' => $this->since, 'until' => $this->until]));
    }
