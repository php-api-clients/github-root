...
use ApiClients\Client\GitHub\Schema\CopilotUsageMetricsDay;
...
final class CopilotMetricsForEnterpriseTeam
{
    ...
    public function __construct(private SchemaValidator $responseSchemaValidator, private Metrics $hydrator, string $enterprise, string $teamSlug, string $since, string $until, int $page = 1, int $perPage = 28)
    {
    ...
    {
        return new Request('GET', (string) (new UriTemplate('/enterprises/{enterprise}/team/{team_slug}/copilot/metrics{?page,per_page,since,until}'))->expand(['enterprise' => $this->enterprise, 'page' => $this->page, 'per_page' => $this->perPage, 'since' => $this->since, 'team_slug' => $this->teamSlug, 'until' => $this->until]));
    }
        ...
        switch ($contentType) {
            case 'application/json':
                ...
                switch ($code) {
                     ...
                     **/
                    case 200:
                        return Observable::fromArray($body, new ImmediateScheduler())->map(function (array $body): CopilotUsageMetricsDay {
                            $error = new RuntimeException();
                            try {
                                $this->responseSchemaValidator->validate($body, Reader::readFromJson(CopilotUsageMetricsDay::SCHEMA_JSON, '\\cebe\\openapi\\spec\\Schema'));

                                return $this->hydrator->hydrateObject(CopilotUsageMetricsDay::class, $body);
