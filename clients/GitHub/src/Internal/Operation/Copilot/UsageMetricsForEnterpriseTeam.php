...
use ApiClients\Client\GitHub\Internal\Hydrator\Operation\Enterprises\Enterprise\Team\TeamSlug\Copilot\Usage;
use ApiClients\Client\GitHub\Schema\BasicError;
use ApiClients\Client\GitHub\Schema\CopilotUsageMetrics;
...
final class UsageMetricsForEnterpriseTeam
{
    ...
    public function __construct(private SchemaValidator $responseSchemaValidator, private Usage $hydrator, string $enterprise, string $teamSlug, string $since, string $until, int $page = 1, int $perPage = 28)
    {
    ...
    {
        return new Request('GET', (string) (new UriTemplate('/enterprises/{enterprise}/team/{team_slug}/copilot/usage{?page,per_page,since,until}'))->expand(['enterprise' => $this->enterprise, 'page' => $this->page, 'per_page' => $this->perPage, 'since' => $this->since, 'team_slug' => $this->teamSlug, 'until' => $this->until]));
    }
        ...
        switch ($contentType) {
            case 'application/json':
                ...
                switch ($code) {
                     ...
                     **/
                    case 200:
                        return Observable::fromArray($body, new ImmediateScheduler())->map(function (array $body): CopilotUsageMetrics {
                            $error = new RuntimeException();
                            try {
                                $this->responseSchemaValidator->validate($body, Reader::readFromJson(CopilotUsageMetrics::SCHEMA_JSON, '\\cebe\\openapi\\spec\\Schema'));

                                return $this->hydrator->hydrateObject(CopilotUsageMetrics::class, $body);
