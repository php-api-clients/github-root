...
final class CopilotMetricsForOrganization
{
    ...
    public function __construct(private SchemaValidator $responseSchemaValidator, private Metrics $hydrator, string $org, string $since, string $until, int $page = 1, int $perPage = 28)
    {
        $this->org                     = $org;
        $this->since                   = $since;
        $this->until                   = $until;
    ...
    {
        return new Request('GET', (string) (new UriTemplate('/orgs/{org}/copilot/metrics{?page,per_page,since,until}'))->expand(['org' => $this->org, 'page' => $this->page, 'per_page' => $this->perPage, 'since' => $this->since, 'until' => $this->until]));
    }
