...
use ApiClients\Client\GitHub\Internal\Hydrator\Operation\Orgs\Org\Team\TeamSlug\Copilot\Metrics;
...
final class CopilotMetricsForTeam
{
    ...
    public function __construct(private SchemaValidator $responseSchemaValidator, private Metrics $hydrator, string $org, string $teamSlug, string $since, string $until, int $page = 1, int $perPage = 28)
    {
        $this->org                     = $org;
    ...
    {
        return new Request('GET', (string) (new UriTemplate('/orgs/{org}/team/{team_slug}/copilot/metrics{?page,per_page,since,until}'))->expand(['org' => $this->org, 'page' => $this->page, 'per_page' => $this->perPage, 'since' => $this->since, 'team_slug' => $this->teamSlug, 'until' => $this->until]));
    }
