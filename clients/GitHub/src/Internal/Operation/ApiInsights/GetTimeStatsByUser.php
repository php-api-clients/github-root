...
final class GetTimeStatsByUser
{
    ...
    public function __construct(private SchemaValidator $responseSchemaValidator, private UserId $hydrator, string $org, string $userId, string $minTimestamp, string $maxTimestamp, string $timestampIncrement)
    {
        $this->org                     = $org;
        $this->userId                  = $userId;
    ...
    {
        return new Request('GET', (string) (new UriTemplate('/orgs/{org}/insights/api/time-stats/users/{user_id}{?max_timestamp,min_timestamp,timestamp_increment}'))->expand(['max_timestamp' => $this->maxTimestamp, 'min_timestamp' => $this->minTimestamp, 'org' => $this->org, 'timestamp_increment' => $this->timestampIncrement, 'user_id' => $this->userId]));
    }
