...
class Configcheck implements ObjectMapper
{
    ...
    private function hydrateApiClients⚡️Client⚡️GitHubEnterprise⚡️Schema⚡️ConfigurationStatus(array $payload): ConfigurationStatus
    {
        $properties    = [];
        $missingFields = [];
        try {
        ...
        if (count($missingFields) > 0) {
            throw UnableToHydrateObject::dueToMissingFields(ConfigurationStatus::class, $missingFields, stack: $this->hydrationStack);
        }

        try {
            return new ConfigurationStatus(...$properties);
        } catch (Throwable $exception) {
            throw UnableToHydrateObject::dueToError('ApiClients\Client\GitHubEnterprise\Schema\ConfigurationStatus', $exception, stack: $this->hydrationStack);
        }
    }
    ...
    {
        assert($object instanceof ConfigurationStatus);
        $result = [];
        ...
        if ($progressSerializer0 === null) {
            $progressSerializer0 = new SerializeArrayItems(...[]);
        }
