...
final class Apps
{
    ...
    public function revokeGrantForApplication(array $params)
    {
        $arguments = array();
        if (array_key_exists('client_id', $params) === false) {
            throw new \InvalidArgumentException('Missing mandatory field: client_id');
        ...
        $operation = new Operation\Apps\RevokeGrantForApplication($arguments['client_id'], $arguments['access_token']);
        $request = $operation->createRequest($params);
    ...
    public function revokeAuthorizationForApplication(array $params)
    {
        $arguments = array();
        if (array_key_exists('client_id', $params) === false) {
            throw new \InvalidArgumentException('Missing mandatory field: client_id');
        ...
        $arguments['access_token'] = $params['access_token'];
        unset($params['access_token']);
        $operation = new Operation\Apps\RevokeAuthorizationForApplication($arguments['client_id'], $arguments['access_token']);
        $request = $operation->createRequest($params);
        return $this->browser->request($request->getMethod(), (string) $request->getUri(), $request->withHeader('Authorization', $this->authentication->authHeader())->getHeaders(), (string) $request->getBody())->then(function (\Psr\Http\Message\ResponseInterface $response) use($operation) : \Psr\Http\Message\ResponseInterface {
            return $operation->createResponse($response);
        });
