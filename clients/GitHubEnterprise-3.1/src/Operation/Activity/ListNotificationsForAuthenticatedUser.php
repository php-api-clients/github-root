...
final class ListNotificationsForAuthenticatedUser
{
    ...
    public function __construct(\League\OpenAPIValidation\Schema\SchemaValidator $responseSchemaValidator, Hydrator\Operation\Notifications $hydrator, string $since, string $before, bool $all = false, bool $participating = false, int $perPage = 30, int $page = 1)
    {
        $this->since = $since;
        $this->before = $before;
        $this->all = $all;
    ...
    {
        return new \RingCentral\Psr7\Request(self::METHOD, \str_replace(array('{since}', '{before}', '{all}', '{participating}', '{per_page}', '{page}'), array($this->since, $this->before, $this->all, $this->participating, $this->perPage, $this->page), self::PATH . '?since={since}&before={before}&all={all}&participating={participating}&per_page={per_page}&page={page}'));
    }
    /**
