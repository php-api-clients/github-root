...
final class ListNotificationsForAuthenticatedUserListing
{
    ...
    public function __construct(private readonly SchemaValidator $responseSchemaValidator, private readonly Internal\Hydrator\Operation\Notifications $hydrator, string $since, string $before, bool $all = false, bool $participating = false, int $perPage = 30, int $page = 1)
    {
        $this->since         = $since;
        $this->before        = $before;
        $this->all           = $all;
    ...
    {
        return new Request('GET', str_replace(['{since}', '{before}', '{all}', '{participating}', '{per_page}', '{page}'], [$this->since, $this->before, $this->all, $this->participating, $this->perPage, $this->page], '/notifications' . '?since={since}&before={before}&all={all}&participating={participating}&per_page={per_page}&page={page}'));
    }
