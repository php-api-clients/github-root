...
final class ListRepoNotificationsForAuthenticatedUser
{
    ...
    public function __construct(private readonly SchemaValidator $responseSchemaValidator, private readonly Hydrator\Operation\Repos\Owner\Repo\Notifications $hydrator, private string $owner, private string $repo, string $since, string $before, bool $all = false, bool $participating = false, int $perPage = 30, int $page = 1)
    {
        $this->since         = $since;
        $this->before        = $before;
        $this->all           = $all;
