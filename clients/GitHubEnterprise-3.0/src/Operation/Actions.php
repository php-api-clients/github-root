...
final class Actions
{
    ...
    public function reRunWorkflow(string $owner, string $repo, int $runId): Schema\Operations\Actions\ReRunWorkflow\Response\ApplicationJson\Created
    {
        if (array_key_exists(Operator\Actions\ReRunWorkflow::class, $this->operator) === false) {
            $this->operator[Operator\Actions\ReRunWorkflow::class] = new Operator\Actions\ReRunWorkflow($this->browser, $this->authentication, $this->responseSchemaValidator, $this->hydrators->getObjectMapperOperation🌀Repos🌀Owner🌀Repo🌀Actions🌀Runs🌀RunId🌀Rerun());
        }

        return $this->operator[Operator\Actions\ReRunWorkflow::class]->call($owner, $repo, $runId);
    }
