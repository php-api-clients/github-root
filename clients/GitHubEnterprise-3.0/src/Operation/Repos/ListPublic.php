...
final class ListPublic
{
    ...
    public function __construct(\League\OpenAPIValidation\Schema\SchemaValidator $responseSchemaValidator, Hydrator\Operation\Repositories $hydrator, int $since, string $visibility = 'public')
    {
        $this->since = $since;
    ...
    {
        return new \RingCentral\Psr7\Request(self::METHOD, \str_replace(array('{since}', '{visibility}'), array($this->since, $this->visibility), self::PATH . '?since={since}&visibility={visibility}'));
    }
    /**
     * @return \Rx\Observable<Schema\MinimalRepository>|Schema\ValidationError
     */
                ...
                switch ($contentType) {
                    case 'application/json':
                        ...
                        return \Rx\Observable::fromArray($body, new \Rx\Scheduler\ImmediateScheduler())->map(function (array $body) : Schema\MinimalRepository {
                            return $this->hydrator->hydrateObject('Schema\\MinimalRepository', $body);
                        });
                }
                break;
            /**Validation failed**/
