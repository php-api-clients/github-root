...
final class ListWebhookDeliveries
{
    ...
    public function __construct(\League\OpenAPIValidation\Schema\SchemaValidator $responseSchemaValidator, Hydrator\Operation\Orgs\CbOrgRcb\Hooks\CbHookIdRcb\Deliveries $hydrator, string $org, int $hook_id, string $cursor, int $per_page = 30)
    {
        $this->org = $org;
    ...
    {
        return new \RingCentral\Psr7\Request(self::METHOD, \str_replace(array('{org}', '{hook_id}', '{cursor}', '{per_page}'), array($this->org, $this->hook_id, $this->cursor, $this->per_page), self::PATH . '?cursor={cursor}&per_page={per_page}'));
    }
    /**
