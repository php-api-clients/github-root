...
final class ListWebhookDeliveries
{
    ...
    public function __construct(\League\OpenAPIValidation\Schema\SchemaValidator $responseSchemaValidator, Hydrator\Operation\Orgs\CbOrgRcb\Hooks\CbHookIdRcb\Deliveries $hydrator, string $org, int $hookId, string $cursor, int $perPage = 30)
    {
        $this->org = $org;
        $this->hookId = $hookId;
        $this->cursor = $cursor;
    ...
    {
        return new \RingCentral\Psr7\Request(self::METHOD, \str_replace(array('{org}', '{hook_id}', '{cursor}', '{per_page}'), array($this->org, $this->hookId, $this->cursor, $this->perPage), self::PATH . '?cursor={cursor}&per_page={per_page}'));
    }
    /**
