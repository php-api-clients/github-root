...
final class ListWebhookDeliveries
{
    ...
    public function __construct(\League\OpenAPIValidation\Schema\SchemaValidator $responseSchemaValidator, Hydrator\Operation\Repos\CbOwnerRcb\CbRepoRcb\Hooks\CbHookIdRcb\Deliveries $hydrator, string $owner, string $repo, int $hookId, string $cursor, int $perPage = 30)
    {
        $this->owner = $owner;
        $this->repo = $repo;
        $this->hookId = $hookId;
    ...
    {
        return new \RingCentral\Psr7\Request(self::METHOD, \str_replace(array('{owner}', '{repo}', '{hook_id}', '{cursor}', '{per_page}'), array($this->owner, $this->repo, $this->hookId, $this->cursor, $this->perPage), self::PATH . '?cursor={cursor}&per_page={per_page}'));
    }
    /**
