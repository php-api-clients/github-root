...
namespace ApiClients\Client\GitHubEnterprise\Internal\Hydrator\Operation\Admin\Ldap\Teams\TeamId;
...
class Mapping implements ObjectMapper
{
    ...
    private function hydrateApiClients⚡️Client⚡️GitHubEnterprise⚡️Schema⚡️LdapMappingTeam(array $payload): LdapMappingTeam
    {
        $properties    = [];
        $missingFields = [];
        try {
        ...
        if (count($missingFields) > 0) {
            throw UnableToHydrateObject::dueToMissingFields(LdapMappingTeam::class, $missingFields, stack: $this->hydrationStack);
        }

        try {
            return new LdapMappingTeam(...$properties);
        } catch (Throwable $exception) {
            throw UnableToHydrateObject::dueToError('ApiClients\Client\GitHubEnterprise\Schema\LdapMappingTeam', $exception, stack: $this->hydrationStack);
        }
    }
    ...
    {
        assert($object instanceof LdapMappingTeam);
        $result = [];
