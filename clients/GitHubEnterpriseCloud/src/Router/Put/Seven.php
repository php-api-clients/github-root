...
final class Seven
{
    ...
    /** @return array{code: int}||(Schema\TeamMembership|array{code: int})|(Schema\EmptyObject|(Observable<Schema\Label>|Schema\BasicError)|(Schema\ScimUser */
                            ...
                            if ($pathChunks[5] === 'customization') {
                                if ($pathChunks[6] === 'issuer') {
                                    if ($call === 'PUT /enterprises/{enterprise}/actions/oidc/customization/issuer') {
                                        ...
                                        return $this->router[Router\Put\Actions::class]->SetActionsOidcCustomIssuerPolicyForEnterprise($params);
                                    }
                                }
                            }
                            ...
                            if ($pathChunks[5] === 'Users') {
                                if ($pathChunks[6] === '{scim_user_id}') {
                                    if ($call === 'PUT /scim/v2/organizations/{org}/Users/{scim_user_id}') {
                                        $matched = true;
                                        if (array_key_exists(Router\Put\Scim::class, $this->router) === false) {
                                            $this->router[Router\Put\Scim::class] = new Router\Put\Scim($this->requestSchemaValidator, $this->responseSchemaValidator, $this->hydrators, $this->browser, $this->authentication);
                                        }

                                        return $this->router[Router\Put\Scim::class]->SetInformationForProvisionedUser($params);
                                    }
                                }
                            }
                        }
