...
final class Seven
{
    ...
    /** @return array{code:int}|Schema\Operations\EnterpriseAdmin\ListLabelsForSelfHostedRunnerForEnterprise\Response\ApplicationJson\Ok|Schema\EmptyObject|Schema\TeamMembership|Schema\ProtectedBranch|iterable<Schema\Label>|Schema\BasicError|Schema\PullRequestMergeResult|Schema\Operations\Pulls\UpdateBranch\Response\ApplicationJson\Accepted\Application\Json|Schema\ScimUser */
                    ...
                    if ($pathChunks[3] === 'actions') {
                        if ($pathChunks[4] === 'oidc') {
                            if ($pathChunks[5] === 'customization') {
                                if ($pathChunks[6] === 'issuer') {
                                    if ($call === 'PUT /enterprises/{enterprise}/actions/oidc/customization/issuer') {
                                        return $this->routers->internal🔀Router🔀Put🔀Actions()->setActionsOidcCustomIssuerPolicyForEnterprise($params);
                                    }
                                }
                            }
                            ...
                            if ($pathChunks[5] === '{secret_name}') {
                                if ($pathChunks[6] === 'repositories') {
                                    if ($call === 'PUT /orgs/{org}/codespaces/secrets/{secret_name}/repositories') {
                                        return $this->routers->internal🔀Router🔀Put🔀Codespaces()->setSelectedReposForOrgSecret($params);
                                    }
                                }
                            }
                        }
                            ...
                            if ($pathChunks[5] === 'secrets') {
                                if ($pathChunks[6] === '{secret_name}') {
                                    if ($call === 'PUT /repos/{owner}/{repo}/codespaces/secrets/{secret_name}') {
                                        return $this->routers->internal🔀Router🔀Put🔀Codespaces()->createOrUpdateRepoSecret($params);
                                    }
                                }
                            }
                        ...
                        if ($pathChunks[4] === '{org}') {
                            if ($pathChunks[5] === 'Users') {
                                if ($pathChunks[6] === '{scim_user_id}') {
                                    if ($call === 'PUT /scim/v2/organizations/{org}/Users/{scim_user_id}') {
                                        return $this->routers->internal🔀Router🔀Put🔀Scim()->setInformationForProvisionedUser($params);
                                    }
                                }
                            }
                        }
                            ...
                            if ($pathChunks[5] === 'repositories') {
                                if ($pathChunks[6] === '{repository_id}') {
                                    if ($call === 'PUT /user/codespaces/secrets/{secret_name}/repositories/{repository_id}') {
                                        return $this->routers->internal🔀Router🔀Put🔀Codespaces()->addRepositoryForSecretForAuthenticatedUser($params);
                                    }
                                }
                            }
                        }
