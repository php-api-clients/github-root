...
final class CopilotMetricsForEnterpriseListing
{
    public const OPERATION_ID    = 'copilot/copilot-metrics-for-enterprise';
    public const OPERATION_MATCH = 'LIST /enterprises/{enterprise}/copilot/metrics';
    ...
    public function __construct(private readonly SchemaValidator $responseSchemaValidator, private readonly Internal\Hydrator\Operation\Enterprises\Enterprise\Copilot\Metrics $hydrator, string $enterprise, string $since, string $until, int $page = 1, int $perPage = 28)
    {
    ...
    {
        return new Request('GET', str_replace(['{enterprise}', '{since}', '{until}', '{page}', '{per_page}'], [$this->enterprise, $this->since, $this->until, $this->page, $this->perPage], '/enterprises/{enterprise}/copilot/metrics' . '?since={since}&until={until}&page={page}&per_page={per_page}'));
    }
