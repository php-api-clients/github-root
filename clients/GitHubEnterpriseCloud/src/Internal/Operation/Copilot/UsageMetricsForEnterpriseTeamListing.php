...
final class UsageMetricsForEnterpriseTeamListing
{
    public const OPERATION_ID    = 'copilot/usage-metrics-for-enterprise-team';
    public const OPERATION_MATCH = 'LIST /enterprises/{enterprise}/team/{team_slug}/copilot/usage';
    ...
    public function __construct(private readonly SchemaValidator $responseSchemaValidator, private readonly Internal\Hydrator\Operation\Enterprises\Enterprise\Team\TeamSlug\Copilot\Usage $hydrator, string $enterprise, string $teamSlug, string $since, string $until, int $page = 1, int $perPage = 28)
    {
    ...
    {
        return new Request('GET', str_replace(['{enterprise}', '{team_slug}', '{since}', '{until}', '{page}', '{per_page}'], [$this->enterprise, $this->teamSlug, $this->since, $this->until, $this->page, $this->perPage], '/enterprises/{enterprise}/team/{team_slug}/copilot/usage' . '?since={since}&until={until}&page={page}&per_page={per_page}'));
    }
