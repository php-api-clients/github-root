...
final class UsageMetricsForTeam
{
    public const OPERATION_ID    = 'copilot/usage-metrics-for-team';
    public const OPERATION_MATCH = 'GET /orgs/{org}/team/{team_slug}/copilot/usage';
    ...
    public function __construct(private readonly SchemaValidator $responseSchemaValidator, private readonly Internal\Hydrator\Operation\Orgs\Org\Team\TeamSlug\Copilot\Usage $hydrator, string $org, string $teamSlug, string $since, string $until, int $page = 1, int $perPage = 28)
    {
        $this->org      = $org;
    ...
    {
        return new Request('GET', str_replace(['{org}', '{team_slug}', '{since}', '{until}', '{page}', '{per_page}'], [$this->org, $this->teamSlug, $this->since, $this->until, $this->page, $this->perPage], '/orgs/{org}/team/{team_slug}/copilot/usage' . '?since={since}&until={until}&page={page}&per_page={per_page}'));
    }
