...
final class UsageMetricsForOrgListing
{
    public const OPERATION_ID    = 'copilot/usage-metrics-for-org';
    public const OPERATION_MATCH = 'LIST /orgs/{org}/copilot/usage';
    ...
    public function __construct(private readonly SchemaValidator $responseSchemaValidator, private readonly Internal\Hydrator\Operation\Orgs\Org\Copilot\Usage $hydrator, string $org, string $since, string $until, int $page = 1, int $perPage = 28)
    {
        $this->org     = $org;
        $this->since   = $since;
        $this->until   = $until;
    ...
    {
        return new Request('GET', str_replace(['{org}', '{since}', '{until}', '{page}', '{per_page}'], [$this->org, $this->since, $this->until, $this->page, $this->perPage], '/orgs/{org}/copilot/usage' . '?since={since}&until={until}&page={page}&per_page={per_page}'));
    }
