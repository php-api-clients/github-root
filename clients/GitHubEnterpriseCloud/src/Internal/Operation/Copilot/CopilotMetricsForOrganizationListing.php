...
final class CopilotMetricsForOrganizationListing
{
    public const OPERATION_ID    = 'copilot/copilot-metrics-for-organization';
    public const OPERATION_MATCH = 'LIST /orgs/{org}/copilot/metrics';
    ...
    public function __construct(private readonly SchemaValidator $responseSchemaValidator, private readonly Internal\Hydrator\Operation\Orgs\Org\Copilot\Metrics $hydrator, string $org, string $since, string $until, int $page = 1, int $perPage = 28)
    {
        $this->org     = $org;
        $this->since   = $since;
        $this->until   = $until;
    ...
    {
        return new Request('GET', str_replace(['{org}', '{since}', '{until}', '{page}', '{per_page}'], [$this->org, $this->since, $this->until, $this->page, $this->perPage], '/orgs/{org}/copilot/metrics' . '?since={since}&until={until}&page={page}&per_page={per_page}'));
    }
