...
final class ListAccountsForPlanListing
{
    ...
    public function __construct(private readonly SchemaValidator $responseSchemaValidator, private readonly Internal\Hydrator\Operation\MarketplaceListing\Plans\PlanId\Accounts $hydrator, int $planId, string $direction, string $sort = 'created', int $perPage = 30, int $page = 1)
    {
        $this->planId    = $planId;
    ...
    {
        return new Request('GET', str_replace(['{plan_id}', '{direction}', '{sort}', '{per_page}', '{page}'], [$this->planId, $this->direction, $this->sort, $this->perPage, $this->page], '/marketplace_listing/plans/{plan_id}/accounts' . '?direction={direction}&sort={sort}&per_page={per_page}&page={page}'));
    }
