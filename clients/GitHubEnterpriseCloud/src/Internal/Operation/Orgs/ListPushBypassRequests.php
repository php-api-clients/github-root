...
final class ListPushBypassRequests
{
    ...
    public const OPERATION_MATCH = 'GET /orgs/{org}/bypass-requests/push-rules';
    ...
    public function __construct(private readonly SchemaValidator $responseSchemaValidator, private readonly Internal\Hydrator\Operation\Orgs\Org\BypassRequests\PushRules $hydrator, string $org, string $repositoryName, string $reviewer, string $requester, string $timePeriod = 'day', string $requestStatus = 'all', int $perPage = 30, int $page = 1)
    {
        $this->org            = $org;
    ...
    {
        return new Request('GET', str_replace(['{org}', '{repository_name}', '{reviewer}', '{requester}', '{time_period}', '{request_status}', '{per_page}', '{page}'], [$this->org, $this->repositoryName, $this->reviewer, $this->requester, $this->timePeriod, $this->requestStatus, $this->perPage, $this->page], '/orgs/{org}/bypass-requests/push-rules' . '?repository_name={repository_name}&reviewer={reviewer}&requester={requester}&time_period={time_period}&request_status={request_status}&per_page={per_page}&page={page}'));
    }
