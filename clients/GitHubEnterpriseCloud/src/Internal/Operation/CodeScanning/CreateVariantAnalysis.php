...
final class CreateVariantAnalysis
{
    ...
    public function __construct(private readonly SchemaValidator $requestSchemaValidator, private readonly SchemaValidator $responseSchemaValidator, private readonly Internal\Hydrator\Operation\Repos\Owner\Repo\CodeScanning\Codeql\VariantAnalyses $hydrator, string $owner, string $repo)
    {
        $this->owner = $owner;
        $this->repo  = $repo;
    }
    ...
    {
        $this->requestSchemaValidator->validate($data, Reader::readFromJson(Schema\CodeScanning\CreateVariantAnalysis\Request\ApplicationJson::SCHEMA_JSON, \cebe\openapi\spec\Schema::class));

        return new Request('POST', str_replace(['{owner}', '{repo}'], [$this->owner, $this->repo], '/repos/{owner}/{repo}/code-scanning/codeql/variant-analyses'), ['Content-Type' => 'application/json'], json_encode($data));
    }
