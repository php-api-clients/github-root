...
final class RemoveResourceFromCostCenter
{
    ...
    {
        $this->requestSchemaValidator->validate($data, Reader::readFromJson(Schema\Billing\RemoveResourceFromCostCenter\Request\ApplicationJson::SCHEMA_JSON, \cebe\openapi\spec\Schema::class));

        return new Request('DELETE', str_replace(['{enterprise}', '{cost_center_id}'], [$this->enterprise, $this->costCenterId], '/enterprises/{enterprise}/settings/billing/cost-centers/{cost_center_id}/resource'), ['Content-Type' => 'application/json'], json_encode($data));
    }

    public function createResponse(ResponseInterface $response): Schema\Operations\Billing\RemoveResourceFromCostCenter\Response\ApplicationJson\Ok\Application\Json
    {
        ...
        switch ($contentType) {
            case 'application/json':
                ...
                switch ($code) {
                     ...
                     **/
                    case 200:
                        $this->responseSchemaValidator->validate($body, Reader::readFromJson(Schema\Operations\Billing\RemoveResourceFromCostCenter\Response\ApplicationJson\Ok\Application\Json::SCHEMA_JSON, \cebe\openapi\spec\Schema::class));

                        return $this->hydrator->hydrateObject(Schema\Operations\Billing\RemoveResourceFromCostCenter\Response\ApplicationJson\Ok\Application\Json::class, $body);
                    /**
                     * Bad Request
                     **/
