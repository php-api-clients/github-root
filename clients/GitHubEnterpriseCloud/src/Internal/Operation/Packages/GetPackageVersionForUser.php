...
final class GetPackageVersionForUser
{
    ...
    public function __construct(private readonly SchemaValidator $responseSchemaValidator, private readonly Internal\Hydrator\Operation\Users\Username\Packages\PackageType\PackageName\Versions\PackageVersionId $hydrator, string $packageType, string $packageName, int $packageVersionId, string $username)
    {
    ...
    {
        return new Request('GET', str_replace(['{package_type}', '{package_name}', '{package_version_id}', '{username}'], [$this->packageType, $this->packageName, $this->packageVersionId, $this->username], '/users/{username}/packages/{package_type}/{package_name}/versions/{package_version_id}'));
    }
