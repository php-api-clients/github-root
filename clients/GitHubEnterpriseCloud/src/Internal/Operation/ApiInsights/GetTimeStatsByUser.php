...
final class GetTimeStatsByUser
{
    ...
    public function __construct(private readonly SchemaValidator $responseSchemaValidator, private readonly Internal\Hydrator\Operation\Orgs\Org\Insights\Api\TimeStats\Users\UserId $hydrator, string $org, string $userId, string $minTimestamp, string $maxTimestamp, string $timestampIncrement)
    {
        $this->org                = $org;
        $this->userId             = $userId;
    ...
    {
        return new Request('GET', str_replace(['{org}', '{user_id}', '{min_timestamp}', '{max_timestamp}', '{timestamp_increment}'], [$this->org, $this->userId, $this->minTimestamp, $this->maxTimestamp, $this->timestampIncrement], '/orgs/{org}/insights/api/time-stats/users/{user_id}' . '?min_timestamp={min_timestamp}&max_timestamp={max_timestamp}&timestamp_increment={timestamp_increment}'));
    }
