...
class CostCenters implements ObjectMapper
{
    ...
    public function hydrateObject(string $className, array $payload): object
    {
        return match ($className) {
            'ApiClients\Client\GitHubEnterpriseCloud\Schema\GetAllCostCenters' => $this->hydrateApiClients⚡️Client⚡️GitHubEnterpriseCloud⚡️Schema⚡️GetAllCostCenters($payload),
    ...
    private function hydrateApiClients⚡️Client⚡️GitHubEnterpriseCloud⚡️Schema⚡️GetAllCostCenters(array $payload): GetAllCostCenters
    {
        $properties    = [];
        $missingFields = [];
        try {
            $value = $payload['cost_centers'] ?? null;

            if ($value === null) {
                $properties['costCenters'] = null;
                goto after_costCenters;
            }

            $properties['costCenters'] = $value;

            after_costCenters:
        } catch (Throwable $exception) {
            throw UnableToHydrateObject::dueToError('ApiClients\Client\GitHubEnterpriseCloud\Schema\GetAllCostCenters', $exception, stack: $this->hydrationStack);
        }

        if (count($missingFields) > 0) {
            throw UnableToHydrateObject::dueToMissingFields(GetAllCostCenters::class, $missingFields, stack: $this->hydrationStack);
        }

        try {
            return new GetAllCostCenters(...$properties);
        } catch (Throwable $exception) {
            throw UnableToHydrateObject::dueToError('ApiClients\Client\GitHubEnterpriseCloud\Schema\GetAllCostCenters', $exception, stack: $this->hydrationStack);
        }
    }
    ...
    public function serializeObjectOfType(object $object, string $className): mixed
    {
        try {
            return match ($className) {
                ...
                'ApiClients\Client\GitHubEnterpriseCloud\Schema\GetAllCostCenters' => $this->serializeObjectApiClients⚡️Client⚡️GitHubEnterpriseCloud⚡️Schema⚡️GetAllCostCenters($object),
    ...
    private function serializeObjectApiClients⚡️Client⚡️GitHubEnterpriseCloud⚡️Schema⚡️GetAllCostCenters(mixed $object): mixed
    {
        assert($object instanceof GetAllCostCenters);
        $result = [];
        ...
        static $costCentersSerializer0;

        if ($costCentersSerializer0 === null) {
            $costCentersSerializer0 = new SerializeArrayItems(...[]);
        }

        $costCenters                                      = $costCentersSerializer0->serialize($costCenters, $this);
        after_costCenters:        $result['cost_centers'] = $costCenters;
