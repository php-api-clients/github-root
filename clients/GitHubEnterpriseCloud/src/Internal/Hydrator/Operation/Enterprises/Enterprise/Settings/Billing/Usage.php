...
class Usage implements ObjectMapper
{
    ...
    public function hydrateObject(string $className, array $payload): object
    {
        return match ($className) {
            'ApiClients\Client\GitHubEnterpriseCloud\Schema\BillingUsageReport' => $this->hydrateApiClients⚡️Client⚡️GitHubEnterpriseCloud⚡️Schema⚡️BillingUsageReport($payload),
    ...
    private function hydrateApiClients⚡️Client⚡️GitHubEnterpriseCloud⚡️Schema⚡️BillingUsageReport(array $payload): BillingUsageReport
    {
        $properties    = [];
        $missingFields = [];
        try {
            $value = $payload['usage_items'] ?? null;

            if ($value === null) {
                $properties['usageItems'] = null;
                goto after_usageItems;
            }

            $properties['usageItems'] = $value;

            after_usageItems:
        } catch (Throwable $exception) {
            throw UnableToHydrateObject::dueToError('ApiClients\Client\GitHubEnterpriseCloud\Schema\BillingUsageReport', $exception, stack: $this->hydrationStack);
        }

        if (count($missingFields) > 0) {
            throw UnableToHydrateObject::dueToMissingFields(BillingUsageReport::class, $missingFields, stack: $this->hydrationStack);
        }

        try {
            return new BillingUsageReport(...$properties);
        } catch (Throwable $exception) {
            throw UnableToHydrateObject::dueToError('ApiClients\Client\GitHubEnterpriseCloud\Schema\BillingUsageReport', $exception, stack: $this->hydrationStack);
        }
    }
    ...
    public function serializeObjectOfType(object $object, string $className): mixed
    {
        try {
            return match ($className) {
                ...
                'ApiClients\Client\GitHubEnterpriseCloud\Schema\BillingUsageReport' => $this->serializeObjectApiClients⚡️Client⚡️GitHubEnterpriseCloud⚡️Schema⚡️BillingUsageReport($object),
    ...
    private function serializeObjectApiClients⚡️Client⚡️GitHubEnterpriseCloud⚡️Schema⚡️BillingUsageReport(mixed $object): mixed
    {
        assert($object instanceof BillingUsageReport);
        $result = [];

        $usageItems = $object->usageItems;

        if ($usageItems === null) {
            goto after_usageItems;
        }

        static $usageItemsSerializer0;

        if ($usageItemsSerializer0 === null) {
            $usageItemsSerializer0 = new SerializeArrayItems(...[]);
        }

        $usageItems                                     = $usageItemsSerializer0->serialize($usageItems, $this);
        after_usageItems:        $result['usage_items'] = $usageItems;
