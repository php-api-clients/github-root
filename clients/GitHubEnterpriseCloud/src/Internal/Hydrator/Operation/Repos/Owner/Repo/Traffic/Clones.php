...
namespace ApiClients\Client\GitHubEnterpriseCloud\Internal\Hydrator\Operation\Repos\Owner\Repo\Traffic;
...
class Clones implements ObjectMapper
{
    ...
    private function hydrateApiClients⚡️Client⚡️GitHubEnterpriseCloud⚡️Schema⚡️CloneTraffic(array $payload): CloneTraffic
    {
        $properties    = [];
        $missingFields = [];
        try {
        ...
        if (count($missingFields) > 0) {
            throw UnableToHydrateObject::dueToMissingFields(CloneTraffic::class, $missingFields, stack: $this->hydrationStack);
        }

        try {
            return new CloneTraffic(...$properties);
        } catch (Throwable $exception) {
            throw UnableToHydrateObject::dueToError('ApiClients\Client\GitHubEnterpriseCloud\Schema\CloneTraffic', $exception, stack: $this->hydrationStack);
        }
    }
    ...
    {
        assert($object instanceof CloneTraffic);
        $result = [];
        ...
        if ($clonesSerializer0 === null) {
            $clonesSerializer0 = new SerializeArrayItems(...[]);
        }
