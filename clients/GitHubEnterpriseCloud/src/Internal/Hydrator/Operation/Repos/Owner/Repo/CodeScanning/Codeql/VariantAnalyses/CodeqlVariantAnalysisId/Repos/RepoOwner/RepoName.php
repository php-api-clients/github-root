...
namespace ApiClients\Client\GitHubEnterpriseCloud\Internal\Hydrator\Operation\Repos\Owner\Repo\CodeScanning\Codeql\VariantAnalyses\CodeqlVariantAnalysisId\Repos\RepoOwner;
...
class RepoName implements ObjectMapper
{
    ...
    public function hydrateObject(string $className, array $payload): object
    {
        return match ($className) {
            'ApiClients\Client\GitHubEnterpriseCloud\Schema\CodeScanningVariantAnalysisRepoTask' => $this->hydrateApiClients⚡️Client⚡️GitHubEnterpriseCloud⚡️Schema⚡️CodeScanningVariantAnalysisRepoTask($payload),
    ...
    private function hydrateApiClients⚡️Client⚡️GitHubEnterpriseCloud⚡️Schema⚡️CodeScanningVariantAnalysisRepoTask(array $payload): CodeScanningVariantAnalysisRepoTask
    {
        $properties    = [];
        $missingFields = [];
        try {
            ...
            $value = $payload['analysis_status'] ?? null;

            if ($value === null) {
                $missingFields[] = 'analysis_status';
                goto after_analysisStatus;
            }

            $properties['analysisStatus'] = $value;

            after_analysisStatus:

            $value = $payload['artifact_size_in_bytes'] ?? null;

            if ($value === null) {
                $properties['artifactSizeInBytes'] = null;
                goto after_artifactSizeInBytes;
            }

            $properties['artifactSizeInBytes'] = $value;

            after_artifactSizeInBytes:

            $value = $payload['result_count'] ?? null;

            if ($value === null) {
                $properties['resultCount'] = null;
                goto after_resultCount;
            }

            $properties['resultCount'] = $value;

            after_resultCount:

            $value = $payload['failure_message'] ?? null;

            if ($value === null) {
                $properties['failureMessage'] = null;
                goto after_failureMessage;
            }

            $properties['failureMessage'] = $value;

            after_failureMessage:

            $value = $payload['database_commit_sha'] ?? null;

            if ($value === null) {
                $properties['databaseCommitSha'] = null;
                goto after_databaseCommitSha;
            }

            $properties['databaseCommitSha'] = $value;

            after_databaseCommitSha:

            $value = $payload['source_location_prefix'] ?? null;

            if ($value === null) {
                $properties['sourceLocationPrefix'] = null;
                goto after_sourceLocationPrefix;
            }

            $properties['sourceLocationPrefix'] = $value;

            after_sourceLocationPrefix:

            $value = $payload['artifact_url'] ?? null;

            if ($value === null) {
                $properties['artifactUrl'] = null;
                goto after_artifactUrl;
            }

            $properties['artifactUrl'] = $value;

            after_artifactUrl:
        } catch (Throwable $exception) {
            throw UnableToHydrateObject::dueToError('ApiClients\Client\GitHubEnterpriseCloud\Schema\CodeScanningVariantAnalysisRepoTask', $exception, stack: $this->hydrationStack);
        }

        if (count($missingFields) > 0) {
            throw UnableToHydrateObject::dueToMissingFields(CodeScanningVariantAnalysisRepoTask::class, $missingFields, stack: $this->hydrationStack);
        }

        try {
            return new CodeScanningVariantAnalysisRepoTask(...$properties);
        } catch (Throwable $exception) {
            throw UnableToHydrateObject::dueToError('ApiClients\Client\GitHubEnterpriseCloud\Schema\CodeScanningVariantAnalysisRepoTask', $exception, stack: $this->hydrationStack);
        }
    }
    ...
    public function serializeObjectOfType(object $object, string $className): mixed
    {
        try {
            return match ($className) {
                ...
                'ApiClients\Client\GitHubEnterpriseCloud\Schema\CodeScanningVariantAnalysisRepoTask' => $this->serializeObjectApiClients⚡️Client⚡️GitHubEnterpriseCloud⚡️Schema⚡️CodeScanningVariantAnalysisRepoTask($object),
    ...
    private function serializeObjectApiClients⚡️Client⚡️GitHubEnterpriseCloud⚡️Schema⚡️CodeScanningVariantAnalysisRepoTask(mixed $object): mixed
    {
        assert($object instanceof CodeScanningVariantAnalysisRepoTask);
        $result = [];
        ...
        $analysisStatus                                         = $object->analysisStatus;
        after_analysisStatus:        $result['analysis_status'] = $analysisStatus;

        $artifactSizeInBytes = $object->artifactSizeInBytes;

        if ($artifactSizeInBytes === null) {
            goto after_artifactSizeInBytes;
        }

        after_artifactSizeInBytes:        $result['artifact_size_in_bytes'] = $artifactSizeInBytes;
        ...
        after_resultCount:        $result['result_count'] = $resultCount;
        ...
        after_failureMessage:        $result['failure_message'] = $failureMessage;

        $databaseCommitSha = $object->databaseCommitSha;

        if ($databaseCommitSha === null) {
            goto after_databaseCommitSha;
        }

        after_databaseCommitSha:        $result['database_commit_sha'] = $databaseCommitSha;

        $sourceLocationPrefix = $object->sourceLocationPrefix;

        if ($sourceLocationPrefix === null) {
            goto after_sourceLocationPrefix;
        }

        after_sourceLocationPrefix:        $result['source_location_prefix'] = $sourceLocationPrefix;

        $artifactUrl = $object->artifactUrl;

        if ($artifactUrl === null) {
            goto after_artifactUrl;
        }

        after_artifactUrl:        $result['artifact_url'] = $artifactUrl;
