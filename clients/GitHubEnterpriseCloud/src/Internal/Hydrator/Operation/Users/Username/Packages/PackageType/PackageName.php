...
namespace ApiClients\Client\GitHubEnterpriseCloud\Internal\Hydrator\Operation\Users\Username\Packages\PackageType;
...
class PackageName implements ObjectMapper
{
    ...
    private function hydrateApiClients⚡️Client⚡️GitHubEnterpriseCloud⚡️Schema⚡️Package(array $payload): Package
    {
        $properties    = [];
        $missingFields = [];
        try {
        ...
        if (count($missingFields) > 0) {
            throw UnableToHydrateObject::dueToMissingFields(Package::class, $missingFields, stack: $this->hydrationStack);
        }
