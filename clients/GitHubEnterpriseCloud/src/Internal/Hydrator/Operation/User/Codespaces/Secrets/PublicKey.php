...
class PublicKey implements ObjectMapper
{
    ...
    private function hydrateApiClients⚡️Client⚡️GitHubEnterpriseCloud⚡️Schema⚡️CodespacesUserPublicKey(array $payload): CodespacesUserPublicKey
    {
        $properties    = [];
        $missingFields = [];
        try {
        ...
        if (count($missingFields) > 0) {
            throw UnableToHydrateObject::dueToMissingFields(CodespacesUserPublicKey::class, $missingFields, stack: $this->hydrationStack);
        }

        try {
            return new CodespacesUserPublicKey(...$properties);
        } catch (Throwable $exception) {
            throw UnableToHydrateObject::dueToError('ApiClients\Client\GitHubEnterpriseCloud\Schema\CodespacesUserPublicKey', $exception, stack: $this->hydrationStack);
        }
    }
    ...
    {
        assert($object instanceof CodespacesUserPublicKey);
        $result = [];
