...
namespace ApiClients\Client\GitHubEnterpriseCloud\Internal\Hydrator\Operation\User\Codespaces\Secrets;
...
class SecretName implements ObjectMapper
{
    ...
    private function hydrateApiClients⚡️Client⚡️GitHubEnterpriseCloud⚡️Schema⚡️CodespacesSecret(array $payload): CodespacesSecret
    {
        $properties    = [];
        $missingFields = [];
        try {
        ...
        if (count($missingFields) > 0) {
            throw UnableToHydrateObject::dueToMissingFields(CodespacesSecret::class, $missingFields, stack: $this->hydrationStack);
        }

        try {
            return new CodespacesSecret(...$properties);
        } catch (Throwable $exception) {
            throw UnableToHydrateObject::dueToError('ApiClients\Client\GitHubEnterpriseCloud\Schema\CodespacesSecret', $exception, stack: $this->hydrationStack);
        }
    }
    ...
    {
        assert($object instanceof CodespacesSecret);
        $result = [];
