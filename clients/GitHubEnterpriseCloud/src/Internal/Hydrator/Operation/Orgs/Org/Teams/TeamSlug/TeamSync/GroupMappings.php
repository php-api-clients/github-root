...
namespace ApiClients\Client\GitHubEnterpriseCloud\Internal\Hydrator\Operation\Orgs\Org\Teams\TeamSlug\TeamSync;
...
class GroupMappings implements ObjectMapper
{
    ...
    private function hydrateApiClients⚡️Client⚡️GitHubEnterpriseCloud⚡️Schema⚡️GroupMapping(array $payload): GroupMapping
    {
        $properties    = [];
        $missingFields = [];
        try {
        ...
        if (count($missingFields) > 0) {
            throw UnableToHydrateObject::dueToMissingFields(GroupMapping::class, $missingFields, stack: $this->hydrationStack);
        }

        try {
            return new GroupMapping(...$properties);
        } catch (Throwable $exception) {
            throw UnableToHydrateObject::dueToError('ApiClients\Client\GitHubEnterpriseCloud\Schema\GroupMapping', $exception, stack: $this->hydrationStack);
        }
    }
    ...
    {
        assert($object instanceof GroupMapping);
        $result = [];
