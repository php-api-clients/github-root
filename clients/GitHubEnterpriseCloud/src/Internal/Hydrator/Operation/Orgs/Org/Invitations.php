...
class Invitations implements ObjectMapper
{
    ...
    private function hydrateApiClients⚡️Client⚡️GitHubEnterpriseCloud⚡️Schema⚡️OrganizationInvitation(array $payload): OrganizationInvitation
    {
        $properties    = [];
        $missingFields = [];
        try {
        ...
        if (count($missingFields) > 0) {
            throw UnableToHydrateObject::dueToMissingFields(OrganizationInvitation::class, $missingFields, stack: $this->hydrationStack);
        }

        try {
            return new OrganizationInvitation(...$properties);
        } catch (Throwable $exception) {
            throw UnableToHydrateObject::dueToError('ApiClients\Client\GitHubEnterpriseCloud\Schema\OrganizationInvitation', $exception, stack: $this->hydrationStack);
        }
    }
    ...
    {
        assert($object instanceof OrganizationInvitation);
        $result = [];
