...
class SecretName implements ObjectMapper
{
    ...
    private function hydrateApiClients⚡️Client⚡️GitHubEnterpriseCloud⚡️Schema⚡️CodespacesOrgSecret(array $payload): CodespacesOrgSecret
    {
        $properties    = [];
        $missingFields = [];
        try {
        ...
        if (count($missingFields) > 0) {
            throw UnableToHydrateObject::dueToMissingFields(CodespacesOrgSecret::class, $missingFields, stack: $this->hydrationStack);
        }

        try {
            return new CodespacesOrgSecret(...$properties);
        } catch (Throwable $exception) {
            throw UnableToHydrateObject::dueToError('ApiClients\Client\GitHubEnterpriseCloud\Schema\CodespacesOrgSecret', $exception, stack: $this->hydrationStack);
        }
    }
    ...
    {
        assert($object instanceof CodespacesOrgSecret);
        $result = [];
