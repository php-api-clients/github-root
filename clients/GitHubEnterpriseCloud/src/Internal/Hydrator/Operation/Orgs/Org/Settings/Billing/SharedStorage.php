...
namespace ApiClients\Client\GitHubEnterpriseCloud\Internal\Hydrator\Operation\Orgs\Org\Settings\Billing;
...
class SharedStorage implements ObjectMapper
{
    ...
    private function hydrateApiClients⚡️Client⚡️GitHubEnterpriseCloud⚡️Schema⚡️CombinedBillingUsage(array $payload): CombinedBillingUsage
    {
        $properties    = [];
        $missingFields = [];
        try {
        ...
        if (count($missingFields) > 0) {
            throw UnableToHydrateObject::dueToMissingFields(CombinedBillingUsage::class, $missingFields, stack: $this->hydrationStack);
        }

        try {
            return new CombinedBillingUsage(...$properties);
        } catch (Throwable $exception) {
            throw UnableToHydrateObject::dueToError('ApiClients\Client\GitHubEnterpriseCloud\Schema\CombinedBillingUsage', $exception, stack: $this->hydrationStack);
        }
    }
    ...
    {
        assert($object instanceof CombinedBillingUsage);
        $result = [];
