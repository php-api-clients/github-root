...
final readonly class GetRepoPushBypassRequest
{
    ...
    public function __construct(private Browser $browser, private AuthenticationInterface $authentication, private SchemaValidator $responseSchemaValidator, private Internal\Hydrator\Operation\Repos\Owner\Repo\BypassRequests\PushRules\BypassRequestNumber $hydrator)
    {
    }
    ...
    public function call(string $owner, string $repo, int $bypassRequestNumber): PushRuleBypassRequest
    {
        ...
        $result    = await($this->browser->request($request->getMethod(), (string) $request->getUri(), $request->withHeader('Authorization', $this->authentication->authHeader())->getHeaders(), (string) $request->getBody())->then(static function (ResponseInterface $response) use ($operation): PushRuleBypassRequest {
