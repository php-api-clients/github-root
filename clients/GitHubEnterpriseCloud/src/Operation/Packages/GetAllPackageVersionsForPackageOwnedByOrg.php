...
final class GetAllPackageVersionsForPackageOwnedByOrg
{
    ...
    public function __construct(\League\OpenAPIValidation\Schema\SchemaValidator $responseSchemaValidator, Hydrator\Operation\Orgs\CbOrgRcb\Packages\CbPackageTypeRcb\CbPackageNameRcb\Versions $hydrator, string $package_type, string $package_name, string $org, int $page = 1, int $per_page = 30, string $state = 'active')
    {
    ...
    {
        return new \RingCentral\Psr7\Request(self::METHOD, \str_replace(array('{package_type}', '{package_name}', '{org}', '{page}', '{per_page}', '{state}'), array($this->package_type, $this->package_name, $this->org, $this->page, $this->per_page, $this->state), self::PATH . '?page={page}&per_page={per_page}&state={state}'));
    }
    /**
