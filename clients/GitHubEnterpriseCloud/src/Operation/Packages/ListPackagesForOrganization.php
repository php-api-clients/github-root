...
final class ListPackagesForOrganization
{
    ...
    public function __construct(\League\OpenAPIValidation\Schema\SchemaValidator $responseSchemaValidator, Hydrator\Operation\Orgs\CbOrgRcb\Packages $hydrator, string $packageType, string $org, string $visibility, int $page = 1, int $perPage = 30)
    {
    ...
    {
        return new \RingCentral\Psr7\Request(self::METHOD, \str_replace(array('{package_type}', '{org}', '{visibility}', '{page}', '{per_page}'), array($this->packageType, $this->org, $this->visibility, $this->page, $this->perPage), self::PATH . '?package_type={package_type}&visibility={visibility}&page={page}&per_page={per_page}'));
    }
    /**
