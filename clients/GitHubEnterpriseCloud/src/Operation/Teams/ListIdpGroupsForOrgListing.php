...
final class ListIdpGroupsForOrgListing
{
    public const OPERATION_ID = 'teams/list-idp-groups-for-org';
    public const OPERATION_MATCH = 'LIST /orgs/{org}/team-sync/groups';
    ...
    public function __construct(\League\OpenAPIValidation\Schema\SchemaValidator $responseSchemaValidator, Hydrator\Operation\Orgs\CbOrgRcb\TeamSync\Groups $hydrator, string $org, string $page, string $q, int $per_page = 30)
    {
        $this->org = $org;
        $this->page = $page;
        $this->q = $q;
    ...
    {
        return new \RingCentral\Psr7\Request(self::METHOD, \str_replace(array('{org}', '{page}', '{q}', '{per_page}'), array($this->org, $this->page, $this->q, $this->per_page), self::PATH . '?page={page}&q={q}&per_page={per_page}'));
    }
    /**
