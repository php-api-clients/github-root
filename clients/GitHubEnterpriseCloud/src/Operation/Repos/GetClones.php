...
final class GetClones
{
    ...
    private readonly Hydrator\Operation\Repos\Owner\Repo\Traffic\Clones $hydrator;

    public function __construct(SchemaValidator $responseSchemaValidator, Hydrator\Operation\Repos\Owner\Repo\Traffic\Clones $hydrator, string $owner, string $repo, string $per = 'day')
    {
        $this->owner                   = $owner;
        $this->repo                    = $repo;
        $this->per                     = $per;
        ...
        switch ($contentType) {
            case 'application/json':
                ...
                switch ($code) {
                     ...
                     **/
                    case 200:
                        $this->responseSchemaValidator->validate($body, Reader::readFromJson(Schema\CloneTraffic::SCHEMA_JSON, \cebe\openapi\spec\Schema::class));
