...
final class CreateOrUpdateRepoSecret
{
    ...
    private readonly Hydrator\Operation\Repos\Owner\Repo\Dependabot\Secrets\SecretName $hydrator;

    public function __construct(SchemaValidator $requestSchemaValidator, SchemaValidator $responseSchemaValidator, Hydrator\Operation\Repos\Owner\Repo\Dependabot\Secrets\SecretName $hydrator, string $owner, string $repo, string $secretName)
    {
    ...
    {
        $this->requestSchemaValidator->validate($data, Reader::readFromJson(Schema\Dependabot\CreateOrUpdateRepoSecret\Request\ApplicationJson::SCHEMA_JSON, \cebe\openapi\spec\Schema::class));
