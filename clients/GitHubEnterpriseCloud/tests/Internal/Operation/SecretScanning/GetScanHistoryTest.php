...
/** @covers \ApiClients\Client\GitHubEnterpriseCloud\Internal\Operation\SecretScanning\GetScanHistory */
final class GetScanHistoryTest extends AsyncTestCase
{
    ...
    public function call_httpCode_503_responseContentType_application_json_zero(): void
    {
        ...
        $result = $client->call(Internal\Operation\SecretScanning\GetScanHistory::OPERATION_MATCH, (static function (array $data): array {
    ...
    {
        $response = new Response(200, ['Content-Type' => 'application/json'], json_encode(json_decode(Schema\SecretScanningScanHistory::SCHEMA_EXAMPLE_DATA, true)));
        $auth     = $this->prophesize(AuthenticationInterface::class);
        $auth->authHeader(Argument::any())->willReturn('Bearer beer')->shouldBeCalled();
        $browser = $this->prophesize(Browser::class);
        $browser->withBase(Argument::any())->willReturn($browser->reveal());
