...
final class RestorePackageForAuthenticatedUserTest extends \WyriHaximus\AsyncTestUtilities\AsyncTestCase
{
    ...
    public function httpCode_404_responseContentType_application_json()
    {
        ...
        $browser->request('POST', '/user/packages/generated_null/generated_null/restore?token=generated_null', \Prophecy\Argument::type('array'), \Prophecy\Argument::any())->willReturn(\React\Promise\resolve($response))->shouldBeCalled();
        $client = new \ApiClients\Client\GitHubEnterpriseCloud\Client($auth->reveal(), $browser->reveal());
        $client->call(\ApiClients\Client\GitHubEnterpriseCloud\Operation\Packages\RestorePackageForAuthenticatedUser::OPERATION_MATCH, (static function (array $data) : array {
            $data['package_type'] = 'generated_null';
            $data['package_name'] = 'generated_null';
            $data['token'] = 'generated_null';
            return $data;
        })(array()));
    }
    /**
