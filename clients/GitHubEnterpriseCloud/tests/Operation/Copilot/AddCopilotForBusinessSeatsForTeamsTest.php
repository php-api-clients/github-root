...
final class AddCopilotForBusinessSeatsForTeamsTest extends AsyncTestCase
{
    ...
    {
        $response = new Response(201, ['Content-Type' => 'application/json'], Schema\Operations\Copilot\AddCopilotForBusinessSeatsForTeams\Response\ApplicationJson\Created::SCHEMA_EXAMPLE_DATA);
        $auth     = $this->prophesize(AuthenticationInterface::class);
        $auth->authHeader(Argument::any())->willReturn('Bearer beer')->shouldBeCalled();
        $browser = $this->prophesize(Browser::class);
        $browser->withBase(Argument::any())->willReturn($browser->reveal());
        $browser->withFollowRedirects(Argument::any())->willReturn($browser->reveal());
        $browser->request('POST', '/orgs/generated/copilot/billing/selected_teams', Argument::type('array'), Schema\Copilot\AddCopilotForBusinessSeatsForTeams\Request\ApplicationJson::SCHEMA_EXAMPLE_DATA)->willReturn(resolve($response))->shouldBeCalled();
        $client = new Client($auth->reveal(), $browser->reveal());
        $result = $client->call(Operation\Copilot\AddCopilotForBusinessSeatsForTeams::OPERATION_MATCH, (static function (array $data): array {
            $data['org'] = 'generated';
    ...
    public function operations_httpCode_201_requestContentType_application_json_responseContentType_application_json_zero(): void
    {
        ...
        $browser->request('POST', '/orgs/generated/copilot/billing/selected_teams', Argument::type('array'), Schema\Copilot\AddCopilotForBusinessSeatsForTeams\Request\ApplicationJson::SCHEMA_EXAMPLE_DATA)->willReturn(resolve($response))->shouldBeCalled();
        $client = new Client($auth->reveal(), $browser->reveal());
        $result = await($client->operations()->copilot()->addCopilotForBusinessSeatsForTeams('generated', json_decode(Schema\Copilot\AddCopilotForBusinessSeatsForTeams\Request\ApplicationJson::SCHEMA_EXAMPLE_DATA, true)));
    }
