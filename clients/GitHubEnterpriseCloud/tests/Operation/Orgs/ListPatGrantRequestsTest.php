...
use ApiClients\Client\GitHubEnterpriseCloud\Operation\Orgs\ListPatGrantRequests;
...
final class ListPatGrantRequestsTest extends AsyncTestCase
{
    ...
    public function httpCode_500_responseContentType_application_json(): void
    {
        ...
        $browser->request('GET', '/organizations/generated_null/personal-access-token-requests?owner=&repository=generated_null&permission=generated_null&last_used_before=1970-01-01T00:00:00+00:00&last_used_after=1970-01-01T00:00:00+00:00&per_page=13&page=13&sort=generated_null&direction=generated_null', Argument::type('array'), Argument::any())->willReturn(resolve($response))->shouldBeCalled();
        $client = new Client($auth->reveal(), $browser->reveal());
        $client->call(ListPatGrantRequests::OPERATION_MATCH, (static function (array $data): array {
    ...
    {
        $response = new Response(200, ['Content-Type' => 'application/json'], '[' . Schema\OrganizationProgrammaticAccessGrantRequest::SCHEMA_EXAMPLE_DATA . ']');
        $auth     = $this->prophesize(AuthenticationInterface::class);
        $auth->authHeader(Argument::any())->willReturn('Bearer beer')->shouldBeCalled();
        $browser = $this->prophesize(Browser::class);
        $browser->withBase(Argument::any())->willReturn($browser->reveal());
