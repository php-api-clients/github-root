...
use ApiClients\Client\GitHubEnterpriseCloud\Operation\Orgs\ListPatGrants;
...
final class ListPatGrantsTest extends AsyncTestCase
{
    ...
    public function httpCode_500_responseContentType_application_json(): void
    {
        ...
        $client->call(ListPatGrants::OPERATION_MATCH, (static function (array $data): array {
    ...
    {
        $response = new Response(200, ['Content-Type' => 'application/json'], '[' . Schema\OrganizationProgrammaticAccessGrant::SCHEMA_EXAMPLE_DATA . ']');
        $auth     = $this->prophesize(AuthenticationInterface::class);
        $auth->authHeader(Argument::any())->willReturn('Bearer beer')->shouldBeCalled();
        $browser = $this->prophesize(Browser::class);
        $browser->withBase(Argument::any())->willReturn($browser->reveal());
