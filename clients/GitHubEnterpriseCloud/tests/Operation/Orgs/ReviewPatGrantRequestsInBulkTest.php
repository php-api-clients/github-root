...
use ApiClients\Client\GitHubEnterpriseCloud\Operation\Orgs\ReviewPatGrantRequestsInBulk;
...
final class ReviewPatGrantRequestsInBulkTest extends AsyncTestCase
{
    ...
    public function httpCode_500_requestContentType_application_json_responseContentType_application_json(): void
    {
        ...
        $browser->request('POST', '/organizations/generated_null/personal-access-token-requests', Argument::type('array'), Schema\Orgs\ReviewPatGrantRequestsInBulk\Request\Applicationjson::SCHEMA_EXAMPLE_DATA)->willReturn(resolve($response))->shouldBeCalled();
        $client = new Client($auth->reveal(), $browser->reveal());
        $client->call(ReviewPatGrantRequestsInBulk::OPERATION_MATCH, (static function (array $data): array {
    ...
    {
        $response = new Response(202, ['Content-Type' => 'application/json'], Schema\Operation\Orgs\ReviewPatGrantRequestsInBulk\Response\Applicationjson\H202::SCHEMA_EXAMPLE_DATA);
        $auth     = $this->prophesize(AuthenticationInterface::class);
        $auth->authHeader(Argument::any())->willReturn('Bearer beer')->shouldBeCalled();
        $browser = $this->prophesize(Browser::class);
        $browser->withBase(Argument::any())->willReturn($browser->reveal());
