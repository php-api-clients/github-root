...
final readonly class CodeownersErrors
{
    ...
    /** @return PromiseInterface<(\ApiClients\Client\GitHubEnterprise\Schema\CodeownersErrors|array)> **/
    public function call(string $owner, string $repo, string $ref): PromiseInterface
    {
        ...
        return $this->browser->request($request->getMethod(), (string) $request->getUri(), $request->withHeader('Authorization', $this->authentication->authHeader())->getHeaders(), (string) $request->getBody())->then(static function (ResponseInterface $response) use ($operation): \ApiClients\Client\GitHubEnterprise\Schema\CodeownersErrors|array {
