                    ...
                    if ($pathChunks[3] === 'custom-repository-roles') {
                        if ($pathChunks[4] === '{role_id}') {
                            if ($call === 'DELETE /orgs/{org}/custom-repository-roles/{role_id}') {
                                return $this->routers->router🔀Delete🔀Orgs()->deleteCustomRepoRole($params);
                            }
                        }
                    ...
                    if ($pathChunks[3] === '{repo}') {
                        if ($pathChunks[4] === 'lfs') {
                            ...
                            if ($call === 'DELETE /repos/{owner}/{repo}/vulnerability-alerts') {
                                return $this->routers->router🔀Delete🔀Repos()->disableVulnerabilityAlerts($params);
                            }
                        }
                    }
                }
                    ...
                    if ($pathChunks[3] === 'Groups') {
                        if ($pathChunks[4] === '{scim_group_id}') {
                            if ($call === 'DELETE /scim/v2/Groups/{scim_group_id}') {
                                return $this->routers->router🔀Delete🔀EnterpriseAdmin()->deleteScimGroupFromEnterprise($params);
                            }
                        }
                    } elseif ($pathChunks[3] === 'Users') {
                        if ($pathChunks[4] === '{scim_user_id}') {
                            if ($call === 'DELETE /scim/v2/Users/{scim_user_id}') {
                                return $this->routers->router🔀Delete🔀EnterpriseAdmin()->deleteUserFromEnterprise($params);
                            }
                        }
                    }
                }
                    ...
                    if ($pathChunks[3] === '{package_type}') {
                        if ($pathChunks[4] === '{package_name}') {
                            if ($call === 'DELETE /user/packages/{package_type}/{package_name}') {
                                return $this->routers->router🔀Delete🔀Packages()->deletePackageForAuthenticatedUser($params);
                            }
                        }
                    }
