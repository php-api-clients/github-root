...
class Announcement implements ObjectMapper
{
    ...
    public function hydrateObject(string $className, array $payload): object
    {
        return match ($className) {
            'ApiClients\Client\GitHubEnterprise\Schema\AnnouncementBanner' => $this->hydrateApiClients⚡️Client⚡️GitHubEnterprise⚡️Schema⚡️AnnouncementBanner($payload),
            default => throw UnableToHydrateObject::noHydrationDefined($className, $this->hydrationStack),
        };
    }

    private function hydrateApiClients⚡️Client⚡️GitHubEnterprise⚡️Schema⚡️AnnouncementBanner(array $payload): AnnouncementBanner
    {
        $properties    = [];
        $missingFields = [];
        try {
        ...
        if (count($missingFields) > 0) {
            throw UnableToHydrateObject::dueToMissingFields(AnnouncementBanner::class, $missingFields, stack: $this->hydrationStack);
        }

        try {
            return new AnnouncementBanner(...$properties);
        } catch (Throwable $exception) {
            throw UnableToHydrateObject::dueToError('ApiClients\Client\GitHubEnterprise\Schema\AnnouncementBanner', $exception, stack: $this->hydrationStack);
        }
    }
    ...
    {
        assert($object instanceof AnnouncementBanner);
        $result = [];
