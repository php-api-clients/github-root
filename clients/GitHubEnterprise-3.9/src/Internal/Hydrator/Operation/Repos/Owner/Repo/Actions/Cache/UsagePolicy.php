...
class UsagePolicy implements ObjectMapper
{
    ...
    private function hydrateApiClients⚡️Client⚡️GitHubEnterprise⚡️Schema⚡️ActionsCacheUsagePolicyForRepository(array $payload): ActionsCacheUsagePolicyForRepository
    {
        $properties    = [];
        $missingFields = [];
        try {
        ...
        if (count($missingFields) > 0) {
            throw UnableToHydrateObject::dueToMissingFields(ActionsCacheUsagePolicyForRepository::class, $missingFields, stack: $this->hydrationStack);
        }

        try {
            return new ActionsCacheUsagePolicyForRepository(...$properties);
        } catch (Throwable $exception) {
            throw UnableToHydrateObject::dueToError('ApiClients\Client\GitHubEnterprise\Schema\ActionsCacheUsagePolicyForRepository', $exception, stack: $this->hydrationStack);
        }
    }
    ...
    {
        assert($object instanceof ActionsCacheUsagePolicyForRepository);
        $result = [];
