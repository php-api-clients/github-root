...
/** @covers \ApiClients\Client\GitHubEnterprise\Operation\Apps\CreateContentAttachment */
final class CreateContentAttachmentTest extends AsyncTestCase
{
    ...
    {
        self::expectException(ErrorSchemas\Operations\Apps\CreateContentAttachment\Response\ApplicationJson\UnsupportedMediaType::class);
        $response = new Response(415, ['Content-Type' => 'application/json'], json_encode(json_decode(Schema\Operations\Apps\CreateContentAttachment\Response\ApplicationJson\UnsupportedMediaType::SCHEMA_EXAMPLE_DATA, true)));
        $auth     = $this->prophesize(AuthenticationInterface::class);
        $auth->authHeader(Argument::any())->willReturn('Bearer beer')->shouldBeCalled();
        $browser = $this->prophesize(Browser::class);
        $browser->withBase(Argument::any())->willReturn($browser->reveal());
