...
final readonly class Hook
{
    ...
    public function __construct(string $type, int $id, string $name, bool $active, array $events, Schema\Hook\Config $config, string $updated_at, string $created_at, string $url, string $test_url, string $ping_url, string $deliveries_url, Schema\HookResponse $last_response)
    {
        $this->type = $type;
        $this->id = $id;
        $this->name = $name;
