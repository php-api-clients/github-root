...
namespace ApiClients\Client\GitHubEnterprise\Internal\Hydrator\Operation\Repos\Owner\Repo\PreReceiveHooks;
...
class PreReceiveHookId implements ObjectMapper
{
    ...
    private function hydrateApiClients⚡️Client⚡️GitHubEnterprise⚡️Schema⚡️RepositoryPreReceiveHook(array $payload): RepositoryPreReceiveHook
    {
        $properties    = [];
        $missingFields = [];
        try {
        ...
        if (count($missingFields) > 0) {
            throw UnableToHydrateObject::dueToMissingFields(RepositoryPreReceiveHook::class, $missingFields, stack: $this->hydrationStack);
        }

        try {
            return new RepositoryPreReceiveHook(...$properties);
        } catch (Throwable $exception) {
            throw UnableToHydrateObject::dueToError('ApiClients\Client\GitHubEnterprise\Schema\RepositoryPreReceiveHook', $exception, stack: $this->hydrationStack);
        }
    }
    ...
    {
        assert($object instanceof RepositoryPreReceiveHook);
        $result = [];
