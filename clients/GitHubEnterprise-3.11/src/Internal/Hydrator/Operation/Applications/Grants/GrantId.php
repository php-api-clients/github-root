...
namespace ApiClients\Client\GitHubEnterprise\Internal\Hydrator\Operation\Applications\Grants;

use ApiClients\Client\GitHubEnterprise\Schema\ApplicationGrant;
use ApiClients\Client\GitHubEnterprise\Schema\ApplicationGrant\App;
...
class GrantId implements ObjectMapper
{
    ...
    private function hydrateApiClients⚡️Client⚡️GitHubEnterprise⚡️Schema⚡️ApplicationGrant(array $payload): ApplicationGrant
    {
        $properties    = [];
        $missingFields = [];
        try {
            ...
            throw UnableToHydrateObject::dueToError('ApiClients\Client\GitHubEnterprise\Schema\ApplicationGrant', $exception, stack: $this->hydrationStack);
        }

        if (count($missingFields) > 0) {
            throw UnableToHydrateObject::dueToMissingFields(ApplicationGrant::class, $missingFields, stack: $this->hydrationStack);
        }

        try {
            return new ApplicationGrant(...$properties);
        } catch (Throwable $exception) {
            throw UnableToHydrateObject::dueToError('ApiClients\Client\GitHubEnterprise\Schema\ApplicationGrant', $exception, stack: $this->hydrationStack);
        }
    }

    private function hydrateApiClients⚡️Client⚡️GitHubEnterprise⚡️Schema⚡️ApplicationGrant⚡️App(array $payload): App
    {
        $properties    = [];
        $missingFields = [];
        try {
    ...
    private function serializeObjectApiClients⚡️Client⚡️GitHubEnterprise⚡️Schema⚡️ApplicationGrant(mixed $object): mixed
    {
        assert($object instanceof ApplicationGrant);
        $result = [];
