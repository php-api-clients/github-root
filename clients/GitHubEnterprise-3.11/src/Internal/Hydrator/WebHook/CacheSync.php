...
use ApiClients\Client\GitHubEnterprise\Schema\WebhookCacheSync;
...
class CacheSync implements ObjectMapper
{
    ...
    public function hydrateObject(string $className, array $payload): object
    {
        return match ($className) {
            'ApiClients\Client\GitHubEnterprise\Schema\WebhookCacheSync' => $this->hydrateApiClients⚡️Client⚡️GitHubEnterprise⚡️Schema⚡️WebhookCacheSync($payload),
    ...
    private function hydrateApiClients⚡️Client⚡️GitHubEnterprise⚡️Schema⚡️WebhookCacheSync(array $payload): WebhookCacheSync
    {
        $properties    = [];
        $missingFields = [];
        try {
            ...
            $value = $payload['cache_location'] ?? null;

            if ($value === null) {
                $missingFields[] = 'cache_location';
                goto after_cacheLocation;
            }

            $properties['cacheLocation'] = $value;
            ...
            throw UnableToHydrateObject::dueToError('ApiClients\Client\GitHubEnterprise\Schema\WebhookCacheSync', $exception, stack: $this->hydrationStack);
        }

        if (count($missingFields) > 0) {
            throw UnableToHydrateObject::dueToMissingFields(WebhookCacheSync::class, $missingFields, stack: $this->hydrationStack);
        }

        try {
            return new WebhookCacheSync(...$properties);
        } catch (Throwable $exception) {
            throw UnableToHydrateObject::dueToError('ApiClients\Client\GitHubEnterprise\Schema\WebhookCacheSync', $exception, stack: $this->hydrationStack);
        }
    }
    ...
    public function serializeObjectOfType(object $object, string $className): mixed
    {
        try {
            return match ($className) {
                ...
                'ApiClients\Client\GitHubEnterprise\Schema\WebhookCacheSync' => $this->serializeObjectApiClients⚡️Client⚡️GitHubEnterprise⚡️Schema⚡️WebhookCacheSync($object),
    ...
    private function serializeObjectApiClients⚡️Client⚡️GitHubEnterprise⚡️Schema⚡️WebhookCacheSync(mixed $object): mixed
    {
        assert($object instanceof WebhookCacheSync);
        $result = [];
        ...
        $cacheLocation                                        = $object->cacheLocation;
        after_cacheLocation:        $result['cache_location'] = $cacheLocation;
