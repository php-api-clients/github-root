...
final class ListAlertsForEnterprise
{
    ...
    public function __construct(\League\OpenAPIValidation\Schema\SchemaValidator $responseSchemaValidator, Hydrator\Operation\Enterprises\CbEnterpriseRcb\SecretScanning\Alerts $hydrator, string $enterprise, string $state, string $secret_type, string $resolution, string $before, string $after, int $per_page = 30)
    {
    ...
    {
        return new \RingCentral\Psr7\Request(self::METHOD, \str_replace(array('{enterprise}', '{state}', '{secret_type}', '{resolution}', '{before}', '{after}', '{per_page}'), array($this->enterprise, $this->state, $this->secret_type, $this->resolution, $this->before, $this->after, $this->per_page), self::PATH . '?state={state}&secret_type={secret_type}&resolution={resolution}&before={before}&after={after}&per_page={per_page}'));
    }
    /**
                ...
                switch ($contentType) {
                    case 'application/json':
                        ...
                        return \Rx\Observable::fromArray($body, new \Rx\Scheduler\ImmediateScheduler())->map(function (array $body) : Schema\OrganizationSecretScanningAlert {
                            return $this->hydrator->hydrateObject('Schema\\OrganizationSecretScanningAlert', $body);
                        });
                }
                break;
            /**Service unavailable**/
