...
final class ListWorkflowRunsForRepo
{
    ...
    public function __construct(private readonly SchemaValidator $responseSchemaValidator, private readonly Internal\Hydrator\Operation\Repos\Owner\Repo\Actions\Runs $hydrator, string $owner, string $repo, string $actor, string $branch, string $event, string $status, string $created, int $checkSuiteId, int $perPage = 30, int $page = 1, bool $excludePullRequests = false)
    {
        $this->owner               = $owner;
        $this->repo                = $repo;
        $this->actor               = $actor;
    ...
    {
        return new Request('GET', str_replace(['{owner}', '{repo}', '{actor}', '{branch}', '{event}', '{status}', '{created}', '{check_suite_id}', '{per_page}', '{page}', '{exclude_pull_requests}'], [$this->owner, $this->repo, $this->actor, $this->branch, $this->event, $this->status, $this->created, $this->checkSuiteId, $this->perPage, $this->page, $this->excludePullRequests], '/repos/{owner}/{repo}/actions/runs' . '?actor={actor}&branch={branch}&event={event}&status={status}&created={created}&check_suite_id={check_suite_id}&per_page={per_page}&page={page}&exclude_pull_requests={exclude_pull_requests}'));
    }
