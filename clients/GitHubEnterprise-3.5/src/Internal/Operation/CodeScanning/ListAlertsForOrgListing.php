...
final class ListAlertsForOrgListing
{
    ...
    public function __construct(private readonly SchemaValidator $responseSchemaValidator, private readonly Internal\Hydrator\Operation\Orgs\Org\CodeScanning\Alerts $hydrator, string $org, string $toolName, string|null $toolGuid, string $before, string $after, string $state, int $page = 1, int $perPage = 30, string $direction = 'desc', string $sort = 'created')
    {
        $this->org       = $org;
    ...
    {
        return new Request('GET', str_replace(['{org}', '{tool_name}', '{tool_guid}', '{before}', '{after}', '{state}', '{page}', '{per_page}', '{direction}', '{sort}'], [$this->org, $this->toolName, $this->toolGuid, $this->before, $this->after, $this->state, $this->page, $this->perPage, $this->direction, $this->sort], '/orgs/{org}/code-scanning/alerts' . '?tool_name={tool_name}&tool_guid={tool_guid}&before={before}&after={after}&state={state}&page={page}&per_page={per_page}&direction={direction}&sort={sort}'));
    }
